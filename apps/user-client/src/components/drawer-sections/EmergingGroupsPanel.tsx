...
function ClusterIcon({ index, reducedMotion }: { index: number; reducedMotion: boolean }) {
  ...
  return (
    <motion.div
      ...
      animate={reducedMotion ? { opacity: 1, scale: 1 } : { scale: 1, opacity: 1 }}
      transition={reducedMotion ? { duration: 0 } : { delay: index * 0.08, duration: 0.35, ease: "easeOut" }}
    >
      <div className="relative w-12 h-12 flex items-center justify-center">
        {/* Soft glow halo */}
        <motion.div
          className="absolute inset-0 rounded-full bg-violet-400/15 dark:bg-violet-400/10"
          animate={reducedMotion ? { scale: 1 } : { scale: [1, 1.18, 1] }}
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex -space-x-1.5">
              {[0, 1, 2].map((i) => (
                <motion.div
                  ...
                  initial={shouldReduceMotion ? { opacity: 0.8, scale: 1 } : { scale: 0, opacity: 0 }}
                  animate={shouldReduceMotion ? { opacity: 0.8 + i * 0.05, scale: 1 } : { scale: 1, opacity: 0.7 + i * 0.1 }}
                  transition={shouldReduceMotion ? { duration: 0 } : { delay: i * 0.1, duration: 0.3, ease: "easeOut" }}
                >
            ...
            <div>
              ...
              <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
                已有 {totalRegistrations} 人报名，满 {minGroupSize} 人后会显现首个小圈
              </p>
            </div>
          </div>
        ) : (
          ...
          <div>
            <div className="flex items-center justify-between mb-3">
              ...
              <motion.div
                ...
                animate={shouldReduceMotion ? { opacity: 1 } : { opacity: [1, 0.65, 1] }}
            ...
            <div className="flex items-center gap-2 flex-wrap">
              {Array.from({ length: renderedClusters }).map((_, i) => (
                <ClusterIcon key={`cluster-${i}`} index={i} reducedMotion={shouldReduceMotion} />
              ))}
