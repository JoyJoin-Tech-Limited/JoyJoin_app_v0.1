...
- **Inline `rpx` styles are silently dropped by the H5 style parser** — any JS-computed inline style (measured geometry, drag-follow positions) must be emitted in `px` (`boundingClientRect` space), never `rpx`. WXSS classes still use rpx as usual.
