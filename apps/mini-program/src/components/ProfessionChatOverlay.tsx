...
function getReactionForProfession(text: string): string {
  ...
  const entries: [string, string][] = [
    ...
    ['律师', '律师的逻辑和表达都很 sharp，跟脑洞型的人碰撞起来特别有意思 ⚖️'],
    ...
    ['编辑', '编辑的文字敏感度和信息整合力，会让你成为局里的高质量听众 ✍️'],
    ...
    ['保险', '保险人的风险意识和长期思维，很适合跟稳重型的人建立连接 🛡️'],
    ...
    ['导游', '导游的表达力和知识储备，很容易在局里成为天然的话题发起者 🗺️'],
    ['空乘', '空乘的服务意识和礼仪感，很容易在局里建立良好的第一印象 ✈️'],
    ['飞行员', '飞行员的决断力和视野，很容易在局里引发别人的好奇 ✈️'],
    ['军人', '军人的纪律性和担当感，很容易在局里建立可靠的信任形象 🎖️'],
    ['警察', '警务背景的正义感和观察力，很容易在局里建立可靠的信任形象 👮'],
    ['消防员', '应急行业的勇敢和责任感，很容易让人产生敬佩和信任 🚒'],
    ['律师助理', '法律背景的严谨和逻辑力，很容易在思辨型话题局里被欣赏 ⚖️'],
    ['法务', '法律背景的严谨和逻辑力，很容易在思辨型话题局里被欣赏 ⚖️'],
    ['法官', '法律背景的严谨和逻辑力，很容易在思辨型话题局里被欣赏 ⚖️'],
    ['检察官', '法律背景的严谨和逻辑力，很容易在思辨型话题局里被欣赏 ⚖️'],
    ...
    ['lawyer', '法律背景的严谨和逻辑力，很容易在思辨型话题局里被欣赏 ⚖️'],
    ...
    ['streamer', '主播的表达力和镜头感，在局里很容易成为话题中心 🎙️'],
    ['actor', '表演行业的人的共情力和表现力，很容易在局里创造深刻连接 🎬'],
    ['pilot', '飞行员的决断力和视野，很容易在局里引发别人的好奇 ✈️'],
    ['flight attendant', '空乘的服务意识和礼仪感，很容易在局里建立良好的第一印象 ✈️'],
    ['soldier', '军人的纪律性和担当感，很容易在局里建立可靠的信任形象 🎖️'],
    ['judge', '法律背景的严谨和逻辑力，很容易在思辨型话题局里被欣赏 ⚖️'],
    ['editor', '编辑的文字敏感度和信息整合力，会让你成为局里的高质量听众 ✍️'],
    ['journalist', '记者的好奇心和提问力，天然适合把浅聊带向深聊 🎤'],
    ['reporter', '记者的好奇心和提问力，天然适合把浅聊带向深聊 🎤'],
    ['brand manager', '品牌人对情绪和趋势的敏感，让你很容易在局里找到共鸣点 🏷️'],
    ['pr', '公关人的情商和应变力，简直是社交局的隐藏 MVP 🎩'],
    ['buyer', '采购的谈判力和资源意识，很容易把弱关系变成强连接 🛒'],
    ['logistics', '供应链人的全局观，让你在看人看事上都更有系统性 🚚'],
    ['architect', '建筑/设计背景的空间感和审美，很容易在文化类局里遇到同好 🏛️'],
    ['accountant', '财务人的严谨和细节控，在局里是那种让人很安心的存在 📒'],
    ['civil servant', '体制内背景的稳定性和表达分寸感，匹配时会优先考虑温和型搭子 🏛️'],
    ['social worker', '社工的共情和利他心，让你在局里很容易收获真诚的反馈 💚'],
    ['professor', '学术背景的深度和表达逻辑，在高质量对话局里特别受欢迎 🎓'],
    ['security', '安保/应急行业的责任感和观察力，很容易让人产生信任感 🛡️'],
    ...
    ['broadcaster', '主播的表达力和镜头感，在局里很容易成为话题中心 🎙️'],
    ['screenwriter', '编剧的故事力和观察力，很容易让对话变得有画面感 ✍️'],
  ...
  for (const [keyword, reaction] of entries) {
...
export default function ProfessionChatOverlay({
  ...
  }, [smartProfession, classificationData, inputValue, onSubmit])
  ...
  return (
    <View className={[
      ...
      <View className='profession-overlay__header'>
        ...
        <View className='profession-overlay__skip' onClick={handleSkip} aria-label='跳过职业输入'>
        ...
        <View className='profession-overlay__chat-inner'>
          {messages.map((msg) => (
            ...
            >
              ...
              <View className={`profession-overlay__bubble profession-overlay__bubble--${msg.sender}`}>
          ...
          {isSubmitting && (
            <View className='profession-overlay__message profession-overlay__message--xiaoyue'>
              <View
                className='profession-overlay__avatar'
                style={{
                  backgroundImage: `url(${getXiaoyueExpressionAsset('loadingSystem')})`,
                }}
              />
      ...
      {showRevealCard && revealTags.length > 0 && (
        <View className='profession-overlay__reveal-card'>
          <Text className='profession-overlay__reveal-title'>你的匹配画像已更新</Text>
          <View className='profession-overlay__reveal-tags'>
            {revealTags.map((tag, i) => (
              <View key={i} className='profession-overlay__reveal-tag'>
                <Text className='profession-overlay__reveal-tag-text'>{tag}</Text>
              </View>
            ))}
          </View>
          <View className='profession-overlay__reveal-confirm' onClick={() => onSubmit(inputValue.trim(), classificationData!)} aria-label='确认职业信息'>
            <Text className='profession-overlay__reveal-confirm-text'>确认</Text>
          </View>
        </View>
      )}
