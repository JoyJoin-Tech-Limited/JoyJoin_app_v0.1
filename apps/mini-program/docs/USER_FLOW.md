...
| **Login (loggedOut state)** | `pages/index/index?auth=logout\|expired` | Settings logout, 401 hard-clear, `useAuthGuard`, onboarding hub | Server-driven `nextStep` redirect after login | `POST /api/auth/wechat/login`, `GET /api/auth/user` | WeChat runtime | Landing `loggedOut` CTA state (retired `pages/login/index` 2026-09-01): welcome-back copy, single 微信一键登录 CTA via `useWeChatLogin` |
