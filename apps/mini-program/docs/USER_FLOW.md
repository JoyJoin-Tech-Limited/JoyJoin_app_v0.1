...
| **Login** | `pages/login/index` | LandingPage CTA, auth-gate, logout | Server-driven `nextStep` redirect | `POST /api/auth/wechat/login`, `GET /api/auth/user` | WeChat runtime | `useWeChatLogin` hook |
...
| **Personality Test** | `pages/onboarding/personality-test/index` | LandingPage, onboarding hub, auth-gate | Test Results (if anon complete) or next onboarding step | `POST /api/assessment/v4/start`, `POST /api/assessment/v4/{id}/answer` | None (supports anonymous) | Adaptive V4 assessment, slider/emoji/choice questions, anonymous session storage |
| **Test Results** | `pages/onboarding/personality-test/results/index` | Personality Test completion | Auth Gate (if anon) or next step (if auth) | `GET /api/assessment/v4/{id}/result` | Completed test session | Slot machine reveal, share poster generation, archetype display |
| **Auth Gate** | `pages/onboarding/personality-test/auth-gate/index` | Test Results (anonymous) | Login import flow or restart | `POST /api/auth/wechat/login-with-test` | Anonymous answers exist | Imports anonymous test into formal account |
...
| **Blind Box Payment** | `pages/blind-box-payment/index` | Pool Registration (entitlement error), Discover CTA, Profile | Payment Verification | `GET /api/pricing`, `GET /api/coupons`, `POST /api/payments/intent`, `POST /api/coupons/validate` | Auth required | Plan selection, coupon validation, pending order resume, payments-disabled state |
