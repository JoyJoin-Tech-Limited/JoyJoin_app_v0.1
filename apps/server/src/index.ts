...
app.use((req, res, next) => {
  ...
  perfStorage.run(metrics, () => {
    res.on("finish", () => {
      ...
      console.log(
        `[RID:${rid}] [${req.method}] ${req.url} - ${res.statusCode} (Total: ${totalMs}ms) | DB Metrics: { count: ${m.dbCount}, total: ${m.dbMs.toFixed(2)}ms, max: ${m.dbMax.toFixed(2)}ms }`
      );
    });
    next();
  });
...
(async () => {
  ...
  if (app.get("env") === "development") {
    ...
    // Production: API only. Static assets served by Nginx/Caddy containers.
    console.log("Production mode: running as API server only.");
  }

  const port = 5000;
  server.listen({ port, host: "0.0.0.0", reuseAddr: true }, () => {
