...
describe('repo cleanup config follow-ups', () => {
  ...
  it('keeps root workspace verification scripts normalized', () => {
    ...
    expect(pkg.scripts['check:clients']).toBe('npm run typecheck -w @joyjoin/shared && npm run typecheck -w @joyjoin/user-client && npm run typecheck -w @joyjoin/admin-client');
