...
import {
  ...
  loadHydratedSessionWithoutProcessAutoAdvance,
  resolveSession,
...
router.post('/:socialSessionId/advance', async (req: any, res) => {
  ...
  const state = await resolveSessionAfterHostAuth(socialSessionId, res, userId, {
    error: 'Only the host can advance phases',
  });
  if (!state) return;
...
router.post('/:socialSessionId/auction/generate-lots', async (req: any, res) => {
  ...
  const state = await resolveSessionAfterHostAuth(socialSessionId, res, userId, {
    error: 'Only the host can generate auction lots',
  });
  if (!state) return;
...
router.post('/:socialSessionId/auction/close-lot', async (req: any, res) => {
  ...
  const state = await resolveSessionAfterHostAuth(socialSessionId, res, userId, {
    error: 'Only the host can close an auction lot',
  });
  if (!state) return;
