...
const npcPatchSchema = z.object({
  ...
  minShiftMinutes: z.number().int().min(90).max(150).optional(),
  maxShiftMinutes: z.number().int().min(90).max(150).optional(),
...
export function registerAdminAlangRoutes(app: Express): void {
  ...
  app.post("/api/admin/alang/npcs", requireAdmin, requireOperatorOrAbove, async (req, res) => {
    const body = z.object({
      ...
      minShiftMinutes: z.number().int().min(90).max(150).default(90),
      maxShiftMinutes: z.number().int().min(90).max(150).default(150),
