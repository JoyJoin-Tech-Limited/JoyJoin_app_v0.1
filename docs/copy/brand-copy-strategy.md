...
### 3.1 权益档位命名方案（2026-08-05 定稿）

> 定价策略不变（DB `pricing_settings` 为准），仅统一展示名。**「权益 / 权益方案」仍是 🔴 档位总称；悦聚卡家族是权益档位的展示名昵称**，与"破冰局/畅聊局/狂欢局"替代"标准局/Premium局"同构。

| planType | 展示名 | EN | 悦仔腔介绍（≤3 语气词） | 徽标 | Legacy（禁） |
|----------|--------|----|------------------------|------|-------------|
| vip_monthly | 悦聚月卡 | YueJu Monthly | 每月 6 场局，说走就走 | — | 月度活动礼包 |
| vip_quarterly | 悦聚季卡 | YueJu Quarterly | 一季 18 场局，场场有人陪 | 更省心 | 季度活动礼包 |
| pack_3 | 三连局包 | Triple Pack | 3 场局名额，先认识三桌新朋友 | 先试试看 | 3次活动包 |
| pack_6 | 六连局包 | Six Pack | 6 场局名额，慢慢玩成常客 | 更灵活 | 6次活动包 |
| event_single | 单场局票 | Single Pass | 一场局的入场券 | — | 单次票 / 单次体验 |

**命名规则：**
- 展示名 = Tone Mode `system-ui`（暖中性、短、可玩味）；介绍文案 = Tone Mode `yuezai-voice`（闺蜜腔，≤3 语气词）。
- 场次数（6/18/3/6）与客户端档位逻辑一致（`blind-box-payment/index.tsx` 按 key 派生），非虚构数字；如 DB 档位语义变更，须同步本表。
- 通知类文案（过期/续费/激活）一律用「悦聚卡」，不得回退到「活动礼包/会员」。
- 档位名称在 `packages/shared/src/copy/terms.ts` 登记 legacy；`单次票` 为禁用词。
