...
**Current limitation:** Shenzhen-only (南山区 + 福田区), no AMap/Gaode geocoding — purely hardcoded client-side district data
...
| 2. Location Selection | ⚠️ Partial | Shenzhen-only; no AMap/Gaode integration; hardcoded district data | **Document as v0.1 limitation** — only Shenzhen pools will show correct districts |
...
- [ ] `AMap/高德 API key` configured if location services needed (not currently used)
