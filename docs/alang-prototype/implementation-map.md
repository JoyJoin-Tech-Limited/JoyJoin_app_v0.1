...
- 搜索不下发隐藏地点坐标、地址、路线、精确距离或距离档位。用户每次主动点击只读取一次 GCJ-02；服务端复用 `TENCENT_MAP_KEY` 做腾讯地图逆地理编码，只有明确的 Tencent 深圳行政区结果才继续，Key 未配置、上游失败、fallback、字段缺失或非深圳均 fail closed，再以 50 米为相遇权威。按用户 + appearance 的 10 分钟 6 次预算由 PostgreSQL 原子计数并跨实例共享；除向腾讯地图 HTTPS API 完成当次逆地理编码所必需的上游传输外，原始用户坐标不写入数据库、日志、审计、分析、JoyJoin 业务 URL 或客户端 query key。班次到点即结束；已解锁对话可继续 24 小时。
...
- 任务只要求在目的地 50 米内点击到达；进店、消费、拍照、扫码、评价、与店员或陌生人互动均非要求。任务反馈为 1–2 道结构化题和可选 100 字私密回信；回信不进入画像、分析、故事、LLM 或后台列表，交付 30 天后清除原文。
- 个性化默认关闭，总开关与人格、兴趣、宽泛行业、行政区、任务行为来源分别可控。拒绝定位则不能参加街头盲盒，且绝不使用 IP 定位回退。
- 当前正式代码由 `routes/domains/alangFlash.ts`、`services/flashService.ts`、`services/flashScheduleService.ts`、`repositories/flashRepo.ts`、`schema/flash.ts` 与后台 `/admin/alang` 共同承载。`alangEnabled` 默认关闭；数据库迁移、seed、逐条人工审核、服务端腾讯地图 Key 与隐私合规确认、readiness、staging 及微信真机验收完成前不得开启。

旧阿浪故事状态机仍作为内部/兼容流程保留；下文涉及 5 米稳定到达、三轮旧剧情、archive 的条目仅描述旧 `missions/:slug` 流程，不得覆盖上述正式街头盲盒契约。FUTURE 04 的“多 NPC 地图”仍不实现：正式版只提供当前在线列表与隐藏地点寻找，不新增探索地图。

## L1. 旧阿浪 V1.7 视觉参考边界

- ACTIVE 03：现有 Discover 街头盲盒入口落为紧凑单 NPC 卡，不新增一级 Tab。
