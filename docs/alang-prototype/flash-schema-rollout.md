...
2. 街头盲盒正式入口默认开放；先执行迁移并确认五位内置 NPC 已存在。迁移中的幂等 seed 生成 15 个 reviewed 单元和 15 个故事碎片，但季保持 draft，因此 readiness 会在正式发布前阻止进入内容链路。
...
5. 运行 `npm run check:flash-readiness -w @joyjoin/server`。门槛要求 1 个已发布季、1 个当前发布快照、15 个已审核单元、至少 5 位角色覆盖、至少 1 个批准相遇地点，并且所有 active NPC 都关联批准相遇地点。
6. 在 staging 完整走通：在线列表 → 选择 NPC → 直接打开前台地图 → 10 米到达 → 故事单元 → 选择 → 碎片 → 同幕任意顺序补齐五人 → 自动进入下一幕 → 15/15 季终。重复提交不得重复发碎片或重复推进。
7. 微信真机检查前台定位停止、长文本滚动、低端机动画、`prefers-reduced-motion`、后台编辑/审核/发布和版本冲突；正式产品没有启用按钮，发布季内容与 readiness 是上线条件。
...
5. 深圳边界资产可以保留作内部参考，但不参与发布 readiness，也不限制用户进入。
   正式安全边界由腾讯地图深圳行政区反查、人工地点审核、排班绑定和服务端 10 米到达校验共同承担。
6. 运行 `npm run check:flash-readiness -w @joyjoin/server`。只有输出 `ready: true`
   才能继续；该命令不写数据库，并要求至少一个批准且启用的相遇地点，且每一位 active NPC
   都关联批准且启用的相遇地点。
...
- 没有批准且启用的相遇地点，或任一 active NPC 未关联批准且启用的相遇地点：readiness 固定失败，用户 API 不得开放。
