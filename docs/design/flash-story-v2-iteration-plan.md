...
### Workstream E — 叙事动作层（2026-08-24 审阅定稿；全季目标已拍板，分两期交付）
| # | 动作 | 决策 | 状态 |
|---|------|------|------|
| E1 | 动作层第一期 sprint（`sprint_20260821_3kmkkw`） | 5 个 v2 试点幕（阿浪×3 + 拾柒×2）纵切：interaction 节点契约对齐 `flashStorySeason.ts` 五种 kind、档案台 MVP、开关 `flashStoryActionsEnabled` 默认 false（ship dark）、埋点扩展既有 6 事件枚举而非新建事件族 | 📋 契约已修订，待 Verifier 验收 |
| E2 | 全季 12 幕启用 sprint（**既定第二期**，2026-08-22 产品拍板全季五 NPC 十五幕动作层） | 入口条件：E1 验收 + C2 真机走查 + C3 基线数据可计算；含 15 幕排列遍历、三次阶段整合与阈值判定 | ⏸ 待 E1 验收后起草 |
| E3 | Atuan 3 幕动作层 | 维持 D3 决策，单独迭代 | ⏸ 未排期 |

> 说明：2026-08-22 契约把范围扩到五位 NPC 十五幕，经 2026-08-24 与产品确认是真实决定，全季目标不变；原稿的问题在于单一 sprint 一次性交付十五幕（且与已提交代码资产口径冲突：模板数、埋点事件族、开关默认值），故拆为 E1/E2 两期 gated 交付。详见契约 Negotiation Log。
