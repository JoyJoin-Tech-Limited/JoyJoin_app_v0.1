...
> **Last updated:** 2026-06-05 (Essential-data onboarding UX hardening: ProfessionChatOverlay added 5s timeout fallback with progressive thinking labels, race-safe timer cleanup, echo suppression, offline detection, device-tier animation gating, haptic reveal feedback, and operational analytics; picker buttons gained dynamic `aria-label`; intent grid memoized for performance; `will-change: transform` added to animated message elements. Discover branded location pill and `LocationFilterDrawer` scroll fix using `980rpx` / `height: 100%`. Custom tab bar layered shadow + pink-tinted center button. Archetype head assets deduplicated without `@3x` to avoid WeChat double-resolution bug).
...
| Page path | Role | Purpose | Notes |
| --- | --- | --- | --- |
| `pages/discover/index` | Primary destination | Shows discoverable event pools with filters, fill state, and the main join entry path. | Launch-primary discovery surface. **Top-of-page hero:** `HeroPromoBanner` — full-bleed Lovart illustration + glass copy panel + breathing CTA + 5 sparkles. Three copy variants (A/B/C) rotate by user archetype + `?promo=A` URL override. WebP→PNG fallback with bounded retry. Kill switch via `user.features.promoBannerEnabled` (env `PROMO_BANNER_ENABLED`). Analytics: `promo_banner_impression`, `promo_banner_cta_tap`, `promo_banner_image_error`, `promo_banner_image_retry`. See `apps/mini-program/src/components/HeroPromoBanner.tsx`. **Smart Regional Discovery** (2026-05-18): silent GPS detection → cluster-level proximity sorting (same cluster first, adjacent next, then time). Manual filter override via `LocationFilterDrawer` (bottom-sheet district selector) with 7-day TTL persistence. **Drawer scroll fix (2026-06-05):** `70dvh` replaced with `980rpx`; `<ScrollView>` forced to `height: 100%` inline + CSS with `min-height: 0` flex parent to unlock scrolling on flex containers. Auto-relaxation banner when manual filter yields empty results. Geo-hint shows detected cluster. Analytics: `geo_detected`, `geo_failed`, `filter_auto_relax`. **Branded location pill (2026-06-05):** discover header pill now renders `$brand-gradient` by default with white text/icon for consistent brand presence. **Interaction polish (2026-06-02):** haptics on pool card tap, location pill open, and pull-to-refresh. **Persistent cache (2026-06-02):** pools data survives app kill via `Taro.setStorageSync` (4h TTL, 75KB cap). Background refetch shows `isFetching` shimmer indicator. Pull-to-refresh evicts persistent cache. **CTA guard (2026-06-04):** `HeroPromoBanner` CTA is disabled when `openPools.length === 0`; tapping shows toast `暂无开放活动` instead of no-op. **Empty state (2026-06-04):** uses `XiaoyueEmptyState` with `emotion='curious'` for brand-consistent emotional support. See `apps/mini-program/src/components/discover/LocationFilterDrawer.tsx`.
| `pages/events/index` | Primary destination | Shows joined events split into upcoming and completed tabs. | User-facing label is `足迹`; page title is `我的足迹`. Primary data source is `GET /api/shell/events` (composite) with fallback to legacy `/api/events/joined`. **Interaction polish (2026-06-02):** haptic feedback on tab switch, `hoverClass` pressed state on tabs, skeleton shimmer loading. **Persistent cache (2026-06-02):** joined events survive app kill via `Taro.setStorageSync` (4h TTL, 75KB cap). Added pull-to-refresh (`usePullDownRefresh`) with cache eviction. Background refetch shows `isFetching` shimmer indicator. **Lovart Batch D (2026-06-04, D1):** `firstEvent` celebrate tile (200rpx) shows the first time a user has a joined event; uses solid gradient bg (no dashed border) for celebratory feel. Also wired to the legacy `pages/my-events/index` alias.
