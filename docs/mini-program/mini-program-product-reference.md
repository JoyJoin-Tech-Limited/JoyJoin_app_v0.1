...
| `pages/onboarding/personality-test/index` | Onboarding step | Runs the adaptive personality assessment with anonymous pre-auth support. | Value-first entry surface. **Lovart Batch D (2026-06-04, D3):** `quizHalfway` cheer tile (160rpx) appears at `progressPercent >= 50` with 0.5s entrance animation + `haptics('light')`; suppresses under `prefers-reduced-motion`. Paired with existing `coachGuide` Xiaoyue mascot. |
