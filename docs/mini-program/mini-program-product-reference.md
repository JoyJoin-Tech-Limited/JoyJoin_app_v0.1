...
| `pages/onboarding/essential-data/index` | Onboarding step | Collects the essential profile fields required before users can continue deeper into onboarding. Includes a Xiaoyue-guided conversational overlay for free-text profession input that auto-classifies professions and generates AI-reveal reactions. | Post-auth step owned by `nextStep`. **Profession input (2026-06-05 hardened):** `ProfessionChatOverlay` is dual-mode (AI-driven via `POST /api/inference/understand-profession` + 184-keyword fallback). **Timeout-aware fallback:** API timeout reduced to 5s; on timeout abort, overlay surfaces a local keyword-matched reaction instead of silently returning. Analytics `profession_chat_classification_fallback` includes `timedOut` flag. **Progressive thinking labels:** `悦仔正在理解你的职业背景…` at 800ms and `还在思考中，马上就好~` at 2800ms. **Race-safe timer cleanup:** `thinkingTimersRef` array is cleared on unmount, new send, success, and catch paths to prevent stale label flash. Added echo suppression for low-quality AI hints (skips hints that mostly repeat user input). Offline detection via `Taro.getNetworkType` guards the API call with graceful toast. Device-tier gating (`profession-overlay--low-end` class) disables entrance animations on degradation-tier devices. Haptic feedback on send, confirm, and reveal card appearance. Analytics events: `profession_chat_echo_suppressed`, `profession_chat_classification_success`, `profession_chat_classification_fallback`, `profession_chat_offline_blocked`. Keyboard scroll-to-bottom uses `bottomAnchorKey` + `ScrollView.scrollIntoView` to keep the input row visible. **Accessibility:** All picker trigger buttons have dynamic `aria-label` reflecting current selection state; intent cards have `aria-pressed` and `aria-label`; chat ScrollView has `aria-live="polite"`. **Performance:** Intent grid is memoized with `useMemo` to prevent re-render on unrelated state changes; animated message elements use `will-change: transform` for GPU compositing. Gated by `SMART_PROFESSION_V1_ENABLED` (default true). |
