...
- `apps/server/src/lib/momentCardRenderer.ts` — server-rendered recap Moment Card PNG (`GET /api/social-icebreaker/:id/moment-card.png`), now the canonical keepsake; the orphaned client-canvas `overlays/MomentCardView.tsx` was removed 2026-09-10 and the PNG is consumed by `RecapKeepsake` in `phases/RecapPhaseView.tsx`
