...
> Intent options and selection logic are defined as a **single source of truth** in `packages/shared/src/constants.ts` (`INTENT_OPTIONS`, `INTENT_FLEXIBLE_OPTION`, `getIntentLabel`, `toggleIntentValue`). `toggleIntentValue(selected, value)` is **uncapped by default** (users may pick any number of explicit intents); selecting every explicit option auto-collapses the selection to `随缘`/flexible. Callers that still need a cap pass `{ maxExplicit: N }` explicitly; the function returns `null` when an explicit cap would be exceeded. All mini-program surfaces (pool-registration, onboarding `essential-data`, edit-profile) are uncapped as of 2026-09-07. Do **not** hardcode intent option arrays or cap logic in individual components — import from these shared constants.
