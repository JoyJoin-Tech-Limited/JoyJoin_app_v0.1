...
4. See anonymized nearby stars — "A 开心柯基 who loves 川菜 is 3 seats away"
...
**What replaces the queue:** The wait period becomes "Your spark is in the mix" — a named, story-driven state where the system periodically drops micro-reveals: "A 太阳鸡 just joined your potential group," "Your compatibility wave is at 78%." The user receives these as gentle push moments, not a progress bar to stare at.
...
3. Thursday evening: receive a Drift notification — "A 🔥 warm dinner is forming near 南山. A 灵感章鱼 who shares 3 interests with you is already in. Join? (expires in 2 hours)"
...
4. If mutual: "You and a 机智狐 chose each other! Your table seed is planted 🌱"
...
| `MicroRevealNotification` | Anonymous hint card for waiting state ("A 太阳鸡 joined your mix") | Low |
...
interface MicroReveal {
  ...
  content: string;      // "A 太阳鸡 joined your mix"
  emoji: string;
  timestamp: string;
}
```
