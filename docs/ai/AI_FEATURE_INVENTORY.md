...
| Social icebreaker personality-dice challenge groups (Choose-Your-Prompt V4) | Same entry point, gated by `PERSONALITY_DICE_CHOOSE_MODE_ENABLED` flag | `generatePersonalityDiceChallengeGroups` in `apps/server/src/socialIcebreakerAIService.ts` | `socialModelRouter` — DeepSeek flash-thinking, `reasoningEffort: 'high'` | Active when flag is ON | `PERSONALITY_DICE_DARES` bank (36 dares, 3 per archetype) |
...
| Occupation free-text search | `apps/mini-program/src/pages/profile-linked/edit-profile/index.tsx` → text input with debounced `POST /api/occupation/search` | `registerOccupationSearchRoutes` in `apps/server/src/routes/domains/occupationSearch.ts` | **Granite** `granite-embedding-97m-multilingual-r2` via self-hosted endpoint; pre-computed occupation vectors in `apps/server/data/occupation-vectors.json` | Active | Exact match vs synonyms first; <0.85 confidence shows suggestions; <0.70 falls back to standard picker |
| Onboarding profession input (overlay) | `apps/mini-program/src/pages/onboarding/essential-data/index.tsx` → `ProfessionChatOverlay` component | `POST /api/inference/understand-profession` → parallel catalog + DeepSeek AI classification → archetype-aware reaction + reveal card with animated tags. **2026-06-10:** `buildReactionHint` conditional: empty string when confidence >= 0.85 or non-AI source; otherwise `generateClarifyingQuestion` (1500ms) produces one warm clarifying question. | `getDeepseekModel('flash')` for reaction + follow-up generation; `classifyIndustryUnified` for classification | Active (gated by `SMART_PROFESSION_ENABLED`, default true) | Falls back to legacy 184-keyword reaction system when feature flag is off or endpoint fails. Catalog classification serves as primary; AI enhances with personalized reaction + display tags. `useProfessionRetry` hook retries unclassified users on next app open. |
...
- Fallback behavior: Uses the deterministic micro-challenge selector (`selectMicroChallenges`, seeded `selector-v1`) on empty, invalid, or failed model output. The curated static bank was removed (W7.5); the selector is always built before the AI call and returned on any failure path.
...
- Description: Creates one personalized live challenge per participant based on dominant trait scores. When `PERSONALITY_DICE_CHOOSE_MODE_ENABLED=true`, generates 3 difficulty-tiered options per player (easy/medium/hard) via `generatePersonalityDiceChallengeGroups`.
...
- AI model/service: `generatePersonalityDiceChallenges(...)` (V1-V3) and `generatePersonalityDiceChallengeGroups(...)` (V4) in `apps/server/src/socialIcebreakerAIService.ts`, routed through `apps/server/src/ai/socialModelRouter.ts`.
- Configuration location: Prompt versions `social-personality-dice-v1` through `social-personality-dice-v4`; phase gating in `apps/server/src/socialIcebreakerPhaseConfig.ts`; choose-mode gated by `PERSONALITY_DICE_CHOOSE_MODE_ENABLED`.
- Current state: Active when the `personality_dice` phase is present in the server-enabled phase list. Choose mode active when flag is ON (default).
- Fallback behavior: Generates a deterministic per-trait challenge from `DICE_CURATED` (V1-V3) or uses `PERSONALITY_DICE_DARES` bank (V4, 36 dares across 12 archetypes) when model output fails.
