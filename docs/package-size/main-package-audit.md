...
> 审计日期：2026-07-15（V1.7 最终干净提交生产构建）
>
> 审计对象：`apps/mini-program/dist` 当前微信生产构建产物；已执行 CDN-only 资源清理
>
> 审计性质：只读；本报告没有迁移、删除或替换任何业务资源
>
> 阿浪边界：不实现 FUTURE 04，不恢复 REMOVED 09，不引入未审批阿浪正式美术。2026-07-15 产品决定另行授权的 Profile-only 像素形象/装备与私人连续故事均位于现有 profile-linked 子包；本审计的历史体积快照不据此重算。

## 1. 结论摘要

当前最终干净提交构建日志中的“3.21 MB”需要先校正口径：

| 口径 | 文件数 | 字节 | MiB | 结论 |
| --- | ---: | ---: | ---: | --- |
| 现有 `check-package-size.mjs` 选取范围 | 211 | 3,370,764 | 3.215 | 这是原始文件总和，不是真实 zip |
| 实际主包文件树（包含 `custom-tab-bar`） | 220 | 3,427,048 | 3.268 | 原始体积，超过 2 MiB |
| 本机 .NET Deflate 估算（包含 `custom-tab-bar`） | 220 | 2,021,770 | 1.928 | 低于 2 MiB，但只剩 75,382 B 余量 |

本机没有 `zip` 命令。现有检查器压缩失败后会回退到原始目录体积，却仍把结果标成 `Main package (zip)`，因此构建日志中的“3.21 MB compressed”并不成立。另一方面，Deflate 结果也不是微信官方上传结果；最终裁决必须由微信开发者工具或 `miniprogram-ci` 的真实上传预检给出。在官方复核之前，本报告仍把主包状态标为 **BLOCK：测量链不可靠且压缩余量过小**。

本快照基于最终将推送的干净提交树生成；用户既有 `squad-unboxing` 工作区改动在测量期间由本地安全快照保护，测量完成后按 SHA-256 原样恢复。范围外文件不会进入本轮阿浪提交，也不会为追求包体数字而被回滚。

阿浪子包本次生产构建为 36 个文件、162,555 B（158.7 KiB），远低于 1.8 MiB 子包门禁。主包问题不是阿浪子包代码本身造成的，主要来源是根级共享 bundle 与被复制到根 `assets/` 的资源。

## 2. 审计方法与边界

主包文件树按以下规则计算：

1. 读取 `dist/app.json`，排除 7 个 `subPackages.root`：`pages/onboarding`、`pages/profile-linked`、`pages/icebreaker-session`、`pages/matching-status`、`pages/pool-registration`、`pages/squad-unboxing`、`pages/alang`。
...
| # | 文件 | 字节 | 分类 | 依赖判断 / 建议 |
| ---: | --- | ---: | --- | --- |
| 1 | `common.js` | 809,593 | 共享 JS | 主包真实共享依赖；不能直接搬走，应拆窄 API barrel 与全局入口后按 bundle diff 验证 |
...
| 8 | `pages/discover/index.js` | 65,168 | JS | tabBar 页面，必须在主包；应拆除不必要的静态依赖 |
| 9 | `assets/mascot/xiaoyue-listening.webp` | 64,186 | 图片 | 由全局 manifest/预加载链保留；可在状态使用审计后改 CDN 或随子包 |
| 10 | `assets/lovart/alang-result-placeholder.webp` | 60,006 | 图片 | 仅阿浪子包使用；可迁阿浪子包或与相同占位图去重 |
| 11 | `assets/lovart/alang-event-card-placeholder.webp` | 60,006 | 图片 | Discover ACTIVE 03 真实依赖；必须留主包或经审批后走 CDN |
| 12 | `assets/lovart/alang-found-scene-placeholder.webp` | 60,006 | 图片 | 仅阿浪子包使用；可迁阿浪子包或与相同占位图去重 |
...
| 15 | `pages/discover/index.wxss` | 41,940 | WXSS | tabBar 页面真实依赖 |
| 16 | `pages/blind-box-payment/index.js` | 41,715 | JS | 非 tab 页；可归入支付/活动子包 |
| 17 | `pages/event-ticket-payment/index.js` | 39,444 | JS | 非 tab 页；可归入支付/活动子包 |
| 18 | `assets/xiaoyue-expressions/xiaoyue-loading-system.webp` | 38,944 | 图片 | 全局 Loading 与 onboarding fallback 使用；可评估 CDN-first 小 fallback |
| 19 | `pages/blind-box-payment/index.wxss` | 30,431 | WXSS | 随支付页面迁移 |
| 20 | `assets/mascot/xiaoyue-thinking.webp` | 28,096 | 图片 | 全局 manifest/预加载链；可按状态拆分或 CDN 化 |
| 21 | `pages/event-ticket-payment/index.wxss` | 24,570 | WXSS | 随支付页面迁移 |
| 22 | `assets/lovart/puzzle/lovart-puzzle-piece-02-20260701-v1.webp` | 24,556 | 图片 | 仅 matching-status 使用，可直接迁现有 matching 子包 |
| 23 | `assets/lovart/puzzle/lovart-puzzle-piece-05-20260701-v1.webp` | 24,230 | 图片 | 仅 matching-status 使用，可直接迁现有 matching 子包 |
| 24 | `assets/lovart/puzzle/lovart-puzzle-piece-04-20260701-v1.webp` | 22,958 | 图片 | 仅 matching-status 使用，可直接迁现有 matching 子包 |
| 25 | `pages/profile/index.js` | 22,935 | JS | tabBar 页面真实依赖；V1.7 数据请求已由 `profileRedesignEnabled` 同步门控 |
| 26 | `pages/profile/index.wxss` | 22,790 | WXSS | tabBar 页面真实依赖 |
...
| 29 | `assets/mascot/xiaoyue-welcome.webp` | 20,838 | 图片 | 全局 manifest/预加载链；可按状态拆分或 CDN 化 |
| 30 | `assets/lovart/squad/squad-host-xiaoyue-fallback.webp` | 19,588 | 图片 | 页面已在 `pages/squad-unboxing` 子包，但 fallback 仍错误留在主包；可随子包迁移 |
...
| 分类 | 文件数 | 字节 | 占实际主包 |
| --- | ---: | ---: | ---: |
| 图片 | 129 | 1,448,017 | 42.25% |
| JS + WXS | 25 | 1,436,483 | 41.92% |
| WXSS | 20 | 346,907 | 10.12% |
| 字体 | 2 | 82,052 | 2.39% |
| WXML | 20 | 72,720 | 2.12% |
| JSON | 22 | 15,245 | 0.44% |
| 意外进入产物的 TypeScript | 2 | 25,624 | 0.75% |
| **合计** | **220** | **3,427,048** | **100%** |

图片细分为 115 个 WebP（1,349,934 B）和 14 个 PNG（98,083 B）；当前主包没有 SVG。路径桶为：根运行时 1,362,820 B、`assets` 1,534,232 B、主包页面 474,155 B、`custom-tab-bar` 55,841 B。
...
| 分析项 | 字节 | 说明 |
| --- | ---: | --- |
| 共享组件/模板 bundle | 1,015,237 | `common.js`、`common.wxss`、`base.wxml`、`comp.*`、`custom-wrapper.*` |
| Taro/vendors runtime | 315,197 | `taro.js`、`vendors.js`、`babelHelpers.js`、`utils.wxs` |
| 完全重复内容 | 121,212 | 三张相同阿浪占位图 + Taro 生成的相同 WXML |
...
- 16 个主包页面的 `index.wxml` 各 80 B 且内容相同，重复开销 1,200 B。这是 Taro 生成模板，不应手工删除，收益也可以忽略。
...
`dist/app.json` 当前把 16 个页面放在主包。以下 5 个在 `tabBar.list`，必须保留主包：
...
`center-hub` 虽由自定义中间按钮展示，仍必须登记在 `tabBar.list`，否则 `switchTab` 会失败。`pages/index` 是启动页，也应留主包。`login` 与其余 9 个非 tab 页可以评估重新分包，但必须保持路由、冷启动、深链和支付恢复行为。`squad-unboxing` 已迁入独立子包，不再计入主包页面。
...
五页还共同引用 `lib/api/api.ts`、`prefetchEngine.ts`、`persistentCache.ts`、`onboardingRoutes.ts`、auth/session、`useCustomTabBarSync`、Loading/JoyJoinIcon 和 `@shared/api`。其中 `@shared/api` 是宽 barrel，`common.js` 已达 809,593 B。应先增加 bundle stats/metafile，再把调用改成领域 subpath；不能在没有差异证据时假设 tree-shaking 已经移除所有无关域。
...
| `squad-host-xiaoyue-fallback.webp` | 19,588 | `pages/squad-unboxing` | 页面已完成分包，但 fallback 仍留在根 `assets` |
...
以下非 tab 页面可按领域重划：payment-verification、blind-box-payment、event-ticket-payment、event-detail、event-feedback、event-coordination、pool-group-detail、center-tab-empty、city-unlock。`login` 需要单独验证冷启动与认证回跳。
...
4. 把 puzzle、auction、custom-tier、pool calendar、squad host fallback 等纯子包资源迁入现有子包。上述高置信资源与测试污染合计可从主包移出约 330,767 B 原始体积。
