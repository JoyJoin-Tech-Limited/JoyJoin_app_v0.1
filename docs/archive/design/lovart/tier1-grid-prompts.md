# Tier 1 — Grid Format Design Prompts

> Generate in **one batch** using 6×6 grid layout  
> Top half (rows 1-3): Archetype Head Icons (40×40px)  
> Bottom half (rows 4-6): Archetype Micro Glyphs (16×16px)

---

## Prompt 1: Complete Tier 1 Icon Grid (6×6)

```
Generate a 6×6 grid of archetype icons for JoyJoin mini-program.

GRID LAYOUT:
- 6 columns × 6 rows
- Each cell: 120×120px with 20px padding between cells
- Top label banner for each cell showing archetype name (Chinese)
- Light gray background for the grid (#F5F5F5)
- White background inside each cell

STYLE LOCK (applies to ALL 36 cells):
- 2D low-poly geometric digital illustration (插画风)
...
- Transparent background inside each cell
- Natural warm palette

═══════════════════════════════════════════════════════════════════
ROWS 1-3: ARCHETYPE HEAD ICONS (40×40px viewBox, scaled to 80px display)
═══════════════════════════════════════════════════════════════════
...
═══════════════════════════════════════════════════════════════════
ROWS 4-6: ARCHETYPE MICRO GLYPHS (16×16px viewBox, scaled to 48px display)
═══════════════════════════════════════════════════════════════════
...
These should feel like premium game character portraits (head icons)
and distinctive game UI glyphs (micro icons). NOT like stock emojis
or generic app icons. The low-poly geometric construction makes them
unmistakably JoyJoin.

═══════════════════════════════════════════════════════════════════
OUTPUT FORMAT:
═══════════════════════════════════════════════════════════════════

- Single image: 6×6 grid, 120px per cell + 20px padding
- Total image size: ~920×920px
- Each cell clearly labeled with archetype name (Chinese)
- Head icons in top half, glyphs in bottom half
- Also provide individual PNG exports for each cell (transparent background)
- Individual exports at: 80×80px (head) and 48×48px (glyph)
```

---

## Prompt 2: Color Token Reference Sheet (3×4 Grid)

```
Generate a 3×4 grid of color palettes for JoyJoin's 12 archetypes.

GRID LAYOUT:
- 4 columns × 3 rows
- Each cell: 280×200px
- Cell shows 5 color swatches in a row + archetype name
- Archetype micro glyph icon in top-right corner of each cell

COLOR SWATCHES PER CELL (left to right):
1. Primary — main brand color for this archetype
2. Light — lighter tint for backgrounds
3. Dark — darker shade for text
4. BG — very light tint for page backgrounds
5. Surface — card/surface background color

═══════════════════════════════════════════════════════════════════
ARCHETYPE COLOR DATA:
═══════════════════════════════════════════════════════════════════
...
- Clean, modern color reference sheet
- Each swatch: 40×40px rounded square with hex code below
- Archetype name in Chinese below swatches
- Light gray grid background
- Professional design system documentation aesthetic

OUTPUT: Single 3×4 reference image + individual cell crops
```

---

## Delivery Checklist for Tier 1

| # | Item | Count | Format | Status |
|---|------|-------|--------|--------|
| 1 | Archetype head icons (individual) | 12 | SVG + PNG 80×80 | ⬜ |
| 2 | Archetype micro glyphs (individual) | 12 | SVG + PNG 48×48 | ⬜ |
| 3 | 6×6 grid reference sheet | 1 | PNG 920×920 | ⬜ |
| 4 | Color token reference sheet | 1 | PNG | ⬜ |
| 5 | Color swatches (individual) | 60 | PNG 40×40 | ⬜ |

**Next:** Once Tier 1 is delivered, we proceed to Tier 2 functional icons.
