...
3. **Phase 2b** — K3 strip pipeline + Profile reuse + return-hook wiring (targets Angle 5). ✅ **2026-08-01: pipeline shipped** — `reveal-strips-manifest.json` + `ArchetypeRevealStrip.tsx` (JS frame-stepping per `XiaoyueSpriteAnimator` pattern), CDN-first → local → static-spritesheet fallback chain, lazy prefetch of only the landed archetype's strip at target-resolution, tier gating (`minimal/emergency` never load strips). **Strips themselves pending K3 generation** (manifest `states` empty by design — pipeline no-ops to static art until assets land). Profile reuse + return-hook wiring not yet built. Targets 22–23.
