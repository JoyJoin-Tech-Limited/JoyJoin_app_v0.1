...
2. **Phase 1+** — storyboard completion **+ F1 blend reframe + Angle 1/2 fixes**. Shipped 2026-07-19: ✅ F1 blend near-miss, ✅ echo whispers, ✅ purpose headline, ✅ flash + ParticleBurst + letter-by-letter (slice 5), ✅ 命格卡 shared generator + poster wiring (slice 4), ✅ return-hook line (slice 6). ✅ **2026-08-01: timing-variant wiring** — `baseline/fast/dramatic` selectable remotely via `personalitySlotProfileFast` / `personalitySlotProfileDramatic` feature flags (dramatic wins; both false = baseline; web-sandbox `?animationProfile=` remains the dev override). Targets 21–22.
3. **Phase 2b** — K3 strip pipeline + Profile reuse + return-hook wiring (targets Angle 5). ✅ **2026-08-01: pipeline shipped** — `reveal-strips-manifest.json` + `ArchetypeRevealStrip.tsx` (JS frame-stepping per JS frame-stepper (mascot sprite animator was deleted 2026-09-03) pattern), CDN-first → local → static-spritesheet fallback chain, lazy prefetch of only the landed archetype's strip at target-resolution, tier gating (`minimal/emergency` never load strips). **Strips themselves pending K3 generation** (manifest `states` empty by design — pipeline no-ops to static art until assets land). Profile reuse + return-hook wiring not yet built. Targets 22–23.
4. **Phase 2c** — **WebGL land-moment spike (PM-endorsed 2026-07-19)** per B1: hybrid architecture (CSS reel + WebGL celebration stage). ✅ **2026-08-01: spike built** — `WebGLLandStage.tsx` + `webglLandStageSession.ts` (three.js r152 via the avatar3d platform adapter, UnrealBloom chain, ~2k accent particles, camera dolly, clearcoat foil card, gyro tilt), gated by `webglRevealEnabled` flag (default false) + `?renderer=webgl` sandbox override + `celebrationTier === 'full'`; any GL failure falls back to the CSS/ParticleBurst celebration with a 6s hard timeout so the flow can never strand. **Remaining: real-device FPS + thermal verification on a mid-tier Android baseline (the last open ship-gate criterion).** Also shipped 2026-08-01: Phase 2a choreography (anticipation reel-shudder + elliptical light-chase comet, CSS-only, tier/reduced-motion-gated). Fake-3D transforms remain the fallback if the spike fails its gate. **❌ 2026-08-11: spike QUARANTINED (deleted)** — the static `three` import inside the onboarding subpackage dragged the avatar3d chunk (`pages/profile-linked/three-avatar.js`) across the subpackage boundary, which WeChat forbids, blanking 原型揭晓 on device (`wx://not-found`). The spike files were removed, the `webglRevealEnabled` flag retired, and `check:avatar3d-import-cone` (wired into `build:weapp`) now guards the cone. The CSS/ParticleBurst celebration (the pre-spike default) is the shipped land moment; revive the WebGL variant only from git history with the three.js cone placed in a permitted package.
5. **Phase 3** — animated share clip (B3, feature-flagged) + rare-variant easter eggs. ✅ **2026-08-01: both built** — ① `shareAnimatedClipEnabled` flag + `POST /api/personality/share-clip` (server-side composition per B3: @napi-rs/canvas frames → ffmpeg → muted MP4 → save-to-album; rate-limited 2/min; SSRF-guarded art URL; ffmpeg added to the API Dockerfile; deterministic seeded frames) + FinalStage 生成动态分享短片 CTA; ② rare-variant easter egg: highly typical match (`isDecisive === true && archetypeConfidence ≥ 0.85`) triggers the 闪光 variant — gold frame, gold flash, all-gold burst rings, coins particle palette, sparkle stars, `闪光命定` hero badge, `slot_rare_variant` analytics. Targets 24.
