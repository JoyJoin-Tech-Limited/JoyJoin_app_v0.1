...
> **Implementation note (2026-07-19, slices 0–3 shipped):** (1) **F1 blend near-miss reframe** — the deterministic 30% near-miss now overshoots onto the user's **secondary archetype** when known (`slotNearMissMode: 'blend'` in `AnimationProfile`, `'random'` retained for rollback), caption `还有一丝别的气息…`; tracked via `slot_near_miss` interaction. (2) **Answer-echo whispers** — during the spin, the progress copy rotates the user's own answer texts (`你说过「…」`, built by `buildEchoWhispers()` from locally stored anonymous answers; authenticated users keep the default caption). (3) **Purpose headline** — slot stage title/copy now states `你的答案正在凝成命格卡` (replaces the "命运转几圈" framing that read as chance). (4) **Instrumentation** — new interactions: `slot_animation_start`, `result_degradation_tier`, `result_stage_dwell` (per-stage dwell on transition), plus existing `skip_animation`. Regression tests: `results/__tests__/echoWhispers.test.ts`.
> **Pipeline note (2026-07-20):** full `pre-ship-pipeline` run over slices 0–6 — report `reports/pre-ship-pipeline-slot-slices-0-6-2026-07-19.md` (completeness 37/44, perf PASS 48/60, harness:gate CONCERN 88/100, SHIP). Blocking fix: `--reduce-motion` class is now wired from `Taro.getSystemInfoSync().reduceMotion` on the results root View (was dead code; also activates the dormant June guards). Spin fallback caption is `正在比对你的选择…`.

> **Implementation note (2026-07-19, slices 4–6 shipped):** (5) **Storyboard-complete celebration** — landed moment now has white flash (`__slot-flash`, full+reduced tiers) + `ParticleBurst` confetti overlay with archetype-accent spotlight (`__slot-burst`, full tier only; reduced tier keeps CSS burst rings) + letter-by-letter hero name reveal in `FinalStage` (`__hero-name-char`, suppressed under reduce-motion). (6) **命格卡 shared generator** — canonical Pokémon-style card in `packages/shared/src/ui/mingCard.ts` (platform-agnostic painter interface) + mini-program adapter `lib/utils/mingCardImage.ts` (legacy CanvasContext pattern) + hidden canvas on the results page; the share poster's hero panel now prefers the generated 命格卡 (`mingCardImagePath`), falling back to raw archetype art. Keywords come from `xiaoyueAnalysis.expressionTags` (≤3), blend line mirrors the hero-card rule. Tests: `results/__tests__/mingCard.test.ts`. (7) **Return hook** — next-horizon line above the primary CTA (`__next-horizon`): guests get the login-gated promise, authenticated users get the discover pointer (soft-truth copy; matching runs in pools). Profile archetype persistence verified pre-existing (`user.archetype ?? user.primaryArchetype`, server-persisted at login-with-test).
