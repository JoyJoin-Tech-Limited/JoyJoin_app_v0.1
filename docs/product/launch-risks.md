...
| R-07 | **No formal data-at-rest encryption** — Database is self-hosted on CVM PostgreSQL. No application-level encryption is applied. | `packages/shared/src/schema.ts` | PII exposure if database credentials are compromised. | Low (disk-level encryption via Tencent Cloud CBS) | Low | Acceptable for internal beta. Review column-level encryption requirements before any public or commercial launch. | ☐ Accepted by: _________  Date: _________ |
