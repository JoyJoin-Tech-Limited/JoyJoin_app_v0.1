...
| R-03 | **Personality-test bypass tool enabled in production** — `apps/server/src/cli/bypassLogin.ts` contains `// TODO: Restrict to development only before production launch` but is documented as currently enabled for internal testing. The script uses a secret key (`ADMIN_CREATE_SECRET_KEY`) but bypasses the full onboarding flow. | `apps/server/src/cli/bypassLogin.ts` | An operator with the `ADMIN_CREATE_SECRET_KEY` value can mark any user as having completed the personality test. If the key leaks, unauthorized bypass is possible. | Low (key required; internal use only) | Medium | Rotate `ADMIN_CREATE_SECRET_KEY` after beta. Disable script in production by gating with `NODE_ENV !== 'production'` check, or remove entirely after beta. | ☐ Accepted by: _________  Date: _________ |
| R-04 | **Viewer role not fully enforced** — The `viewer` admin role is defined in the RBAC system but most routes only check `requireAdmin` (not `requireOperatorOrAbove`). A `viewer` can currently call write endpoints. | `apps/server/src/adminAuth.ts`, `apps/server/src/routes.ts` | A `viewer` account could perform mutations it should not have access to (ban users, override attendance, etc.). | Low (beta cohort is trusted; viewer accounts may not be provisioned yet) | Medium | Acceptable for internal beta if no `viewer` accounts are provisioned. Add `requireOperatorOrAbove` to write endpoints before granting viewer access to untrusted parties. | ☐ Accepted by: _________  Date: _________ |
...
| R-06 | **Legacy user.isAdmin flag fallback in requireAdmin** — `requireAdmin` accepts sessions where `session.userId` points to a user with `isAdmin = true` (the old auth path). These legacy sessions receive `role = 'super_admin'` without going through the `admin_accounts` table. | `apps/server/src/adminAuth.ts` | Legacy admin sessions bypass the new RBAC role/status checks. A legacy admin who should be demoted may retain full access. | Low (legacy path is only for pre-RBAC admins) | Low–Medium | For beta: ensure all active admins log in via the new `POST /api/admin/login` path and have an entry in `admin_accounts`. Deprecate the legacy fallback after all admins have migrated. | ☐ Accepted by: _________  Date: _________ |
...
- [ ] R-03: Gate `bypassLogin.ts` behind `NODE_ENV !== 'production'` or remove it
