...
> Classification of all `.github/skills/` into **ai-runtime** (skills that orchestrate or configure AI services invoked at runtime in the product flow) and **internal** (developer tooling, design guidance, infrastructure, process).
>
> **OpenCode mirror:** `.agents/skills/` contains a parallel skill tree auto-discovered by OpenCode. It is a subset (63 skills) of the canonical `.github/skills/` tree (77 skills). See **Unhandled** in the latest `docs-sync` summary for sync status.
...
| **Frontend guidance** | `frontend-component-architecture`, `frontend-design-audit`, `frontend-hook-engine`, `frontend-performance-and-loading`, `mini-program-frontend-excellence`, `viewport-zero-scroll`, `wow-elements`, `design-system-governance`, `joyjoin-brand-guidelines`, `ui-layout-audit`, `completeness-audit`, `user-satisfaction-audit` |
...
| **Docs & orchestration** | `docs-sync`, `draft-prd`, `first-principles-velocity`, `task-creator`, `pm-sin-mapper`, `agent-coordination-patterns`, `multi-agent-deliberation`, `subagent-context-delegation`, `orchestration-turn-reporting` |
...
| **Communication** | `caveman`, `grill-me` |
...
**Why is `caveman` / `grill-me` internal?**
These are contributor communication aids (plain-language explanation, Socratic questioning). They do not configure runtime AI services.
