...
- **GitHub MCP:** When reviewing architecture decisions that touch open PRs, branch protection rules, or CI pipeline changes, use the **GitHub MCP server** (`github`) to inspect PR context, review history, and workflow definitions rather than inferring from local files alone.
