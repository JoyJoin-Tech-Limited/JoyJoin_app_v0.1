...
| Change the handoff graph, kickoff behavior, tooling sufficiency status, nested subagent workspace behavior, or orchestration skill references | `.github/orchestration.yaml`, `.github/agents/manifest.json`, `.vscode/settings.json`, `.github/ORCHESTRATION.md`, and any impacted agent docs or contributor guidance | `npm run orchestration:validate` and targeted hook/runtime checks |
