...
- Bundled assets: tab-icons, joyjoin-logo, joyjoin-logo-tab, tab-bar-notch-bg, custom-tab-bar, archetype spritesheet (subpackage), locally bundled icon tiers with retina (@1x/@2x/@3x via `JoyJoinIcon`): mood, chemistry, category, intent, expression (rating faces); `status`, `semantic` (info labels), and `ui` ship with @1x/@2x only — their @3x variants are stripped from the production package to stay under WeChat's 2 MB ceiling (3x devices scale the @2x assets). **Archetype heads ship as bare `.webp` filenames** (no `@3x` suffix — see `@3x` pitfall below) and are bundled locally with CDN fallback copies. Landing phase icons (6), empty states, QR code, auction coin icons, Xiaoyue loading + welcome expressions, a curated core of Xiaoyue mascot sprite-sheet fallback (`welcome`, `idle`, `coach`, `loading`, `listening`, `thinking`), Quicksand + Alimama minimal fonts. CDN-primary (with local mirrored fallback): **phase emblems**, reaction, reveal, achievement. CDN-only: remaining mascot sprite states, archetype full-body images (WebP primary + PNG fallback), matching heroes, Lovart illustrations, icebreaker backgrounds, celebration images, extra Xiaoyue expressions, mini-script heroes, ceremony heroes, milestone badges, interest taxonomy v2.0 illustrations (`images/interests/*.webp`). The 2026-06-18 Lovart 5×5 status/UI grid was cropped into `status-icons`, `ui`, `info-labels`, and `reaction-icons` and registered in `emojiToIconMap.ts`.
...
- **WeChat `@3x` image pitfall (2026-06-05):** If a `<Image>` `src` already contains `@3x`, WeChat's runtime can attempt to load `...@3x@3x.webp`, causing 404 / missing image. Fix used for `ArchetypeHead.tsx`: remove the `@3x` suffix from `HEAD_PATHS` and ship duplicate bare-filename `.webp` copies in `src/assets/icons/archetype/`. **Intent icons use the same pattern** (single 144×144 bare `.webp` in `src/assets/icons/intent-icons/`, no `@2x`/`@3x` variants) because the auto-resolution proved unreliable inside the pool-registration subpackage on iOS. Either let WeChat auto-resolve `@2x/@3x` variants, or provide exact copies without the suffix — never hardcode `@3x` into `src`.
...
- **Sprite animator animation engine (2026-06-22):** `XiaoyueSpriteAnimator` uses CSS `steps()` animation via dynamically injected keyframes with CSS custom properties (`--frame-width`, `--columns`, `--current-frame`, `--total-frames`) instead of per-frame `setInterval` + React state updates. No per-frame React state churn. A `pendingExitCleanupRef` pattern defers crossfade cleanup when the app is hidden (handled via `handleAppHide`/`handleAppShow`), preventing stale DOM state on return. The `playKey` remount trigger was removed — `SpriteFrame` no longer remounts on every resolved state change. Background tint `rgba($color-primary, 0.04)` on `&__frame` masks the loading gap before the sprite sheet decodes.
...
**Mini-program profile components (2026-05-24 / layout hardened 2026-06-04 / redesign shipped 2026-06-16 / profile-linked migration 2026-06-17):**
