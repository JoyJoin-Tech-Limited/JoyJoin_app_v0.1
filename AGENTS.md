...
> Compact instructions for AI coding agents. Last updated: 2026-06-12
...
- **Swipe-back / foreground recovery (2026-06-13):** `LandingPage.tsx` resets `isPageExiting` via `useResetOnShow` when the page is re-shown (e.g., iOS swipe-back from an onboarding page). If the user has an incomplete anonymous assessment session or an authenticated `nextStep` that is still inside onboarding, the primary CTA label switches from `测测我的社交氛围` to `继续创建账户` and routes to the appropriate step instead of restarting from the personality test intro.
