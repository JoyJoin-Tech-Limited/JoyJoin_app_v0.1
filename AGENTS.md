...
**Gathering room (集结房间, 2026-08-07):** pre-event online anteroom for matched pool groups — subpackage `pages/gathering-room`, REST `GET /api/pool-groups/:groupId/room-state`, WS presence via `ROOM_*` events (5s leave grace, 2s poke throttle), attendance source of truth `event_attendance`. Ships dark behind `gatheringRoomEnabled` (env `GATHERING_ROOM_ENABLED`, default `false`); room art is the delivered composite WebP (`ROOM_ART_READY = true`) with CSS placeholder as fallback. Avatars reuse existing V2 `PixelAvatarComposite` with each member's equipped outfit/items. Details: `docs/agent-context/gathering-room.md`.

**SD pixel avatars (2026-08-07, decoupled from gathering room 2026-08-10):** compact chibi asset family (4 integer size tiers, content-hashed WebP) built by `npm run build:sd-avatar -w mini-program` into `src/assets/sd-avatar/v1/` + `sd-avatar-assets-v1.json`; runtime `components/mascot/SDAvatar.tsx` via `ArchetypeHead variant='sd'` behind `sdAvatarEnabled` (env `SD_AVATAR_ENABLED`, default `false`). No longer used for gathering room (reuses V2 `PixelAvatarComposite`); held for future compact-avatar surfaces. Manifest currently holds 12 synthesized placeholders until Lovart art lands; `check:sd-avatar-assets` is NOT yet wired into `build:weapp`. Details: `docs/agent-context/mini-program-assets.md`.
