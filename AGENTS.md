...
> Compact instructions for AI coding agents. Last updated: 2026-08-27
...
| Personality engine, archetypes, assessment, matcher | `personality-system` |
...
- ☐ For mini-program changes that add/move a styled component inside a subpackage: run `npm run build:weapp -w mini-program && npm run verify:subpackage-styles -w mini-program` and confirm no non-empty `sub-common.wxss` is produced
...
**WeChat review posture (2026-08-18):** user-facing copy uses event-booking vocabulary — 排桌中/排桌完成/合拍度/活动画像/轮桌畅聊, never 匹配/社交/灵魂/撮合/AI in visible copy (machine identifiers exempt). AIGC 角标 are fail-closed (render only when content was actually AI-generated). Canonical runbook: [`docs/runbooks/wechat-review-submission.md`](./docs/runbooks/wechat-review-submission.md).
...
npm run verify:subscribe-templates    # WeChat subscribe-message template IDs + field keys vs live API
...
# Gotcha (2026-08-17): packOptions.include in project.config.json is an
# upload ALLOW-LIST — a bundled asset matching no include pattern is silently
# dropped from the uploaded package (confirmed twice: 2026-08-04 Flash
# artwork fix 08b55afe1; 2026-08-17 gathering-room art, which left the scene
# bare on device while H5/dev looked fine). Any NEW bundled asset directory
# needs an include regexp in the same PR, and any asset that must render on
# device should load via localAsset() once bundled (CDN-first adds a failure
# mode without saving package size).
# Gotcha (2026-08-17): cdnAsset() is idempotent for absolute URLs — never
# double-wrap a value that may already be cdnAsset()-resolved, or persistent
# cache / downloadFile calls will 404 silently.
# Protocol/runbook: docs/runbooks/mini-program-asset-delivery.md.
...
- **2026-07-28 GHCR image delivery:** staging images are now pushed to GHCR (`ghcr.io/joyjoin-tech-limited/joyjoin-{api,admin}-staging:<sha>`) by the build job and pulled on the CVM (`STAGING_API_IMAGE`/`STAGING_ADMIN_IMAGE`/`GHCR_TOKEN`), replacing the old ~40min chunked-rsync bundle upload that made the stale-candidate check flaky whenever main moved mid-upload. `deploy-staging.sh` retries `docker pull` (layers resume) and keeps the tar-bundle path as a fallback when `STAGING_API_IMAGE` is unset. First-time CVM pulls download full images (slow, cross-Pacific); steady-state pulls only fetch changed app layers. **2026-08-13 hardening:** pulls are now parallel (API + Admin) with 8m-per-attempt timeouts, and a same-region **Tencent Cloud TCR** delivery mode is implemented — precedence `tcr > ghcr > bundle`, TCR pull failure auto-falls-back to GHCR refs. Activate via `STAGING_TCR_ENABLED=true` + `TCR_REGISTRY`/`TCR_USERNAME`/`TCR_TOKEN` secrets + `TCR_NAMESPACE` var (provisioning runbook: `deployment/README.md` §TCR). SSH `command_timeout` raised 45m→60m to fit the TCR+GHCR fallback budget.
...
| Budget tiers (venue assignment) | `docs/agent-context/budget-tier.md` |
...
| Gathering room (集结房间) | `docs/agent-context/gathering-room.md` |
...
**Personality-test iteration (2026-09-02, PR #634):** question-page bubble now shows a per-question **idle whisper** on entry (client copy pool `idleWhispers.ts`, separate state from `postAnswerCommentary`, suppressed in back-review); the PR-7 celebrate bridge (`?celebrate=1`) is **deleted** — completing shell holds 1100ms, then LoadingStage → 200ms keyed crossfade → slot anticipation; slider anchors carry lean-reactive Lovart icons (`soloRest`/`partyReady`); the slot spritesheet is primed during the quiz (dual-cache) with an opacity-pulse shimmer placeholder; the reel has **fake-3D drum curvature** (`slotCurvature.ts`, transforms/opacity only, tier matrix; 2.5D fallback = `SLOT_CURVATURE_ENABLE_3D = false`). WebGL land-stage remains quarantined. Plan + baseline SQL + device-lab checklist: `docs/deliberations/2026-09-02-personality-test-iteration-plan.md`.
...
**Budget tiers (2026-09-16):** canonical ordered tier registry at `packages/shared/src/budgetTiers.ts` (6 tiers, all `per_person`, namespaced ids `dining_*/drinks_*`, `id≠label`) — the ONLY source of tier identity. A guard ratchet (`scripts/check/check-budget-tiers.mjs` + baseline, in the guardrails chain) fails any NEW hardcoded budget literal outside it. Enforced on write (`lib/budgetTierWrite.ts` tolerant normalize) and read (`venueAssignmentService` normalizes both sides, so legacy↔id mixes still match). Venue capability = `venues.budget_categories` (retagged to ids by migration 0093); registrations store ids in `budget_range`/`bar_budget_range` (retagged 0094); merging them into one `budget_tier_ids` column is a deferred contract step. Strict validation + `min(1)` in `_definitions.ts`/`venues.ts` emits `INVALID_BUDGET_TIER`/`BUDGET_TIER_REQUIRED` (route surfaces the coded error, no cap per PRD:2286 multi-select). Degradation — adjacency by `order` distance, **max one tier** (the `±2→8` gradient was rejected for B4's cap), two-pass rescue for undecidable budgets, `budget_adjacent` disclosure, empty-consensus asymmetry removed, capacity the only hard gate — ships behind `budgetAdjacencyEnabled` (**default OFF**; needs dual-run before enabling). City-level catalog `budgetOptions` rides on `GET /api/event-pools/:id` (coverage-annotated, fail-open, **not** availability-aware — slots stay in assignment). Timezone: `lib/eventDateTime.ts` (+8h + `getUTC*`, TZ-independent); `parseEventDate` re-exported from `venueAssignmentService`. **Blockers:** 深圳 `dining_150_below/150_200/300_500` have no restaurant (whitelist); 弥所/Bruma/Max are ASSUMED `drinks_80_150` (≤150 per-person) pending verification. Details: `docs/agent-context/budget-tier.md`, spec `docs/design/budget-tier-architecture-spec-20260916.md`, plan `docs/design/budget-tier-implementation-plan-20260916.md`.

**Social Icebreaker:** Primary in-event flow is `/icebreaker/:sessionId` → Social Icebreaker. Host-paced (no auto-advance), PhaseHeroCard views, all icebreaker LLM calls wrapped in 6s `raceWithTimeout` hard bound. The mini_script phase surface (generate/assign-roles/reveal-act/vote/reveal-solution/ready + bonus/respond + bonus/sentiment) is mounted at **top-level `/api/miniscript/*`** with `socialSessionId` in the body — never `/api/social-icebreaker/:id/...` (client enforced by `miniscriptClientPathContract.test.ts`, 2026-08-06). Long-running AI generation UI uses the reusable `AiGenerationShell`. **Page split (2026-08-12):** the phase-dispatch JSX lives in `SessionPhaseViews.tsx` and the action-handler layer (`performSocialAction` + topics backoff cluster + tier-switch + all phase handlers) in `hooks/useSocialActions.ts` — keep `index.tsx` under the harness gate's 1800-line fail limit; the structural contract tests assert against the hook source. **S6 group beats (2026-08-12, flag `icebreakerGroupBeatsEnabled`, dark):** state-free `SOCIAL_GROUP_BEAT` WS emission (`lib/socialGroupBeats.ts`) + client haptic dispatch (`viewModels/groupBeatModel.ts`, `trackLastMessage: false` in `useWebSocket`) — the 3s poll stays the sole state truth. **Mini-script generation hard bound (2026-08-12):** the pipeline runs inside a 28s `raceWithTimeout` (`PIPELINE_TIMEOUT_MS`, env `SOCIAL_MINISCRIPT_PIPELINE_TIMEOUT_MS`) — a stalled provider socket settles to `pipeline_timeout` → catalog fallback → terminal `complete`; the server emits 1.5s heartbeat progress (`generating` 15→65, `validating` 70→86) and client polls guard terminal stages + per-submit epoch + progress-value monotonic — never compare `updatedAt` across device/server clocks (device clock skew froze the bar at 5%, fixed 2026-08-13). **MiniScript V2 evidence + motive vote (2026-08-28, flag `miniscriptEvidenceVoteV2Enabled`):** snapshot-gated at mini_script phase entry. Schema: additive optional `act_flow[].evidence[]` (≤2/player/act, id/name/description/iconKey + server-only `evidenceReactions` map roleSlot→text) and framework-level `motiveOptions[]` (3–4 strings; correct motive in server-only `solution.why`). Routes: `POST /api/miniscript/present-evidence` (act sub-stage only, duplicate idempotent), `POST /api/miniscript/confirm-read` (presenter-only, idempotent — releases reaction to group immediately), `POST /api/miniscript/open-motive-vote` (host-only, sets round 2), `POST /api/miniscript/advance-ceremony` (host-only, beats: culprit→honor→advance). Server-gated reaction visibility: `reactionText` omitted from `sanitizeStateForClient` until ≥8s or `readConfirmedAt`; presenter always sees own. Two-round vote: round 1 = culprit guess, round 2 = motive guess (sequential); `/vote` carries `voteRound` + `motiveChoice`; `reveal-solution` returns per-player `{ round1Correct, round2Correct? }`. Recap honor line: dual-correct players named in session recap only (no profile persistence). Offline QC: `npm run validate:miniscript-story` (exit 0/1, `--llm` for critic-revise ≤2 rounds). Details: `docs/agent-context/social-icebreaker.md`.

**DeepSeek V4 reasoning trap (2026-08-11):** `deepseek-v4-flash` reasons by default when a request carries no thinking control — at production `max_tokens` budgets (100–500) the budget burns on `reasoning_content` and `message.content` comes back EMPTY, silently degrading surfaces to curated fallbacks. The openai-node SDK in use does NOT serialize `extra_body` (neither body key nor RequestOptions) — thinking control MUST be top-level (`body.thinking`, `body.reasoning_effort`). `getDeepseekClient()` now injects top-level `thinking: {type:'disabled'}` by default; thinking is opt-in only for `analyzeComplexSemantics`. Never reintroduce `extra_body` thinking control or thinking tiers for JSON-formatting functions. Canonical doc: `docs/ai/AI_MODEL_ROUTING_STRATEGY.md`. Benchmark CLI requires `--env-file=.env` (no ambient key → placeholder-key 401s).

**Gathering room (集结房间, 2026-08-07):** pre-event online anteroom for matched pool groups — subpackage `pages/gathering-room`, REST `GET /api/pool-groups/:groupId/room-state`, WS presence via `ROOM_*` events (5s leave grace, 2s poke throttle), attendance source of truth `event_attendance`. Ships dark behind `gatheringRoomEnabled` (env `GATHERING_ROOM_ENABLED`, default `false`); room art is `room-composite-v2.webp` (repaired 2026-08-27 for window panel mis-registration; v1 retained for rollback; `ROOM_ART_READY = true`) with CSS placeholder as fallback. Avatars reuse existing V2 `PixelAvatarComposite` with each member's equipped outfit/items. **Absent rendering (2026-09-10):** absent members render their V2 avatar as a muted grayscale silhouette at their seat anchor (`__seat-body--absent`, opacity 0.45, static) with pulsing name card — PRD §3 名牌/剪影占位. Rendering nothing for absent members broke the room on device ("only my own avatar renders") because absence is the norm pre-event; `event_attendance` rows exist only after explicit confirm, and WS presence covers only concurrently-connected users. The 2026-08-17 lesson still applies: never pile dimmed avatars at the DOOR — seat-anchored silhouettes are the fix. A live absent→present flip keeps the door walk (silhouette swaps to full-color avatar walking in). **Freeze fix (2026-08-12):** the page previously had 4 early returns BEFORE 3 hooks — first room render crashed React mid-mount and froze the loading screen forever; all hooks now run unconditionally above every early return. Never place a hook below an early return on this page (`rules-of-hooks` is not yet a CI gate). Details: `docs/agent-context/gathering-room.md`.

**Registration error code contract (2026-09-10, recurrence lock):** every server route returning a user-facing error MUST include a machine-readable `code` field in the response body. The mini-program `resolveMessage()` in `useOptimisticRegistration.ts` maps `code` → shared `ErrorCode` template from `packages/shared/src/copy/errorBaselines.ts`. Without a `code`, the error falls through to the generic fallback "出了点问题，稍后再试" — which is often wrong (e.g. "already registered" looked like a failure). **Safety net:** if a `code` is present but unmapped in the client, `resolveMessage` now falls through to the server's Chinese `message` field (CJK-only guard, no English leakage) instead of returning the generic fallback. To add a new user-facing error: (1) server route sets `code: "YOUR_CODE"` on the response, (2) add `YOUR_CODE` to the `ErrorCode` union in `errorBaselines.ts`, (3) add the template to `ERROR_MESSAGE_MAP`, (4) the contract test `apps/server/src/__tests__/registrationErrorCodes.test.ts` verifies all `code: "..."` literals in registration routes have templates. Never let a user-facing 4xx/5xx return without a `code`.

**Cascade-delete for test pool cleanup (2026-09-10, recurrence lock):** `apps/server/src/lib/fkCascadeDelete.ts` (`cascadeDeleteByIds`) discovers non-`ON DELETE CASCADE` FK dependents from `pg_constraint` at runtime and deletes transitive children deepest-first. Use this instead of hand-enumerated child deletes — hand-enumerated lists inevitably miss new FK children (the `2026-09-10` fix: `blind_box_pre_attendance` + 20+ FK children were missing from the manual delete list, causing `23503` constraint violations). `cascadeDeleteByIds` is future-proof: new FK children are auto-discovered. One exception: drizzle-only FKs without a DB constraint (e.g. `event_attendance.blind_box_event_id`) must be explicitly deleted before calling `cascadeDeleteByIds`. Contract test: `apps/server/src/__tests__/fkCascadeDeleteContract.test.ts`.

**Event id families (three):** the mini-program can pass `events.id`, `blind_box_events.id` (squad-unboxing redirects to `event-detail?id=blindBoxEventId`), or `event_pools.id`. **Any route that writes a column FK'd to `events.id` (e.g. `event_feedback`, `connections`) MUST canonicalize first via `resolveCanonicalEventId`** (`apps/server/src/lib/resolveCanonicalEventId.ts`) — inserting a raw blind-box id caused the single-test 局 500 `Failed to create feedback` (`event_feedback_event_id_events_id_fk`, fixed 2026-08-07). Read-only routes (`/participants`) may resolve inline.
...
**WeChat subscribe-message notification strategy (2026-09-16):** 3 templates cover one event lifecycle — 报名结果提醒 (排桌完成 XOR 退款, `WECHAT_SUBSCRIBE_TMPL_MATCH_SUCCESS`), 活动状态提醒 (活动当天早上 XOR 候补, `WECHAT_SUBSCRIBE_TMPL_EVENT_REMINDER`, carries venue), 活动评价提醒 (T+1 回顾+反馈召唤, `WECHAT_SUBSCRIBE_TMPL_RECAP`). Client grant = one 3-template `requestSubscribeMessage` bundle from the registration ceremony「开启活动消息提醒」button; verdicts metered via `subscribe_grant_result`. Server sender `lib/wechatSubscribeMessage.ts` (fail-open, errcode 43101 quiet) + ledger table `subscribe_message_sends` (unique user+moment+pool = idempotency claim) + scheduler `lib/subscribeReminderScheduler.ts` (10-min tick; event_day window T-3h..T-12h during 08:00–22:00, recap window T+20h..T+30h during 09:00–21:00; kill switch `subscribeRemindersEnabled`). Push text is service-only — marketing hooks live on landing pages, never in push copy (WeChat bans marketing in subscribe messages). Deliberately NOT pushed: 报名成功, T-24h, 活动开始, in-room nudges.
...
**双人成行 Duo registration (2026-08-07):** Duo invites reuse the `invitations` track re-scoped to a pool — `invitations.poolId` + `invitationType='duo'` (`event_id` is nullable since then), expiry = pool `preference_lock_at` (fallback `date_time`). Routes in `apps/server/src/routes/domains/duo.ts` (`POST /api/pools/:id/duo-invites` idempotent, `GET /api/pools/:id/duo-status`, public rate-limited `GET /api/duo-invites/:code`); helpers in `apps/server/src/lib/duoInvites.ts`. Binding = an `invitation_uses` row written at registration (both `/api/event-pools/:id/register` and the payment-fulfillment path) when both sides hold registrations in the pool. Matching (`poolMatchingService.ts`, `[DUO]` guards): a bound duo is a hard atomic unit — 2 seats, unit score = mean of both members, MAX 1 duo per group, duo-internal pair excluded from group quality metrics, R1 must hold per member. Fallback is LOCKED variant A 整组顺延: an unplaceable duo stays unmatched together → existing 场次未成行 auto-refund pipeline (no duo-specific notification). Design spec: `docs/design/duo-registration-spec-20260807.md`.

**安心补位 Phase 0 (2026-08-27, both flags dark):** user cancel runs through the shared orchestrator `apps/server/src/lib/poolRegistrationCancel.ts` (parity across `/api/pool-registrations/:id` DELETE + blind-box cancel). Boundary = per-registration `matchStatus='matched'`, never `pool.matchedAt`. Flag `preRevealRefundEnabled` → real full refund (money via `claimPaymentForRefund` atomic claim; credits reversed in-tx before delete — also fixed the credit-redemption FK 500); refund failure aborts without deleting. Flag `noRefundAfterReveal` → no refund + honest group state (atomic `memberCount` decrement, `event_attendance.status='cancelled'`, audit + WeCom, member notices, collapse <4 → stayers refunded via `REFUND_CONTEXTS.collapsed`, exiter never refunded). Server-computed `cancelPolicy` drives the mini-program's matched-state cancel affordance + policy-aware dialogs; terms aligned (2026-08-27, penalty paragraph removed — no-show penalties are permanently vetoed). Flags off = exact legacy behavior. Rollout: pre-reveal first, post-reveal after legal sign-off. Phase 1 候补 backfill is designed, not built. Spec: `docs/design/post-reveal-seat-backfill-spec-20260827.md`.
...
**桌友 Card-Deck UI reuse (2026-08-16):** `TablemateCard` (`apps/mini-program/src/components/TablemateCard/index.tsx`) is the single reusable portrait card for matched members across `matching-status` matched carousel, `pool-group-detail` deck strip, and the squad-unboxing front-face reference. Pair-chemistry display helpers (`getPairChemistryWord`, `getPairChemistryTier`, `CHEMISTRY_TIER_EMOJI`, `stripConnectionPointParens`, `shortenConnectionPointForPill`, `buildInterestHookText`) live in `apps/mini-program/src/lib/utils/pairChemistry.ts` and are consumed by all three surfaces. Member detail is surfaced in `TablemateDetailSheet` (drag-to-dismiss, hero-image fade-in, avatar preview, rarity-dotted connection points, `现场见` CTA, `catchMove` scroll containment). New matched-member surfaces must reuse these components rather than inventing a new card variant.
...
- Admin/test-route RBAC is enforced by **server tests, not guardrails**: `apps/server/src/__tests__/adminRbacCoverage.test.ts` covers `/api/admin/*`, and `apps/server/src/__tests__/testAdminAuth.test.ts` covers `/api/test/**` (behavioural route introspection + source scan). The earlier AGENTS.md claim that guardrails itself checked this was inaccurate — it is why finding N1 (`/api/test/admin/*` unauthenticated) slipped through (2026-09-16)
...
- **Diff-scoped design audit (2026-09-01):** `scripts/devtools/design-audit.mjs` supports `--scope full|files|changed`, `--base <ref>` (default `$DESIGN_AUDIT_BASE` → merge-base with origin/main), and `--fail-on none|warning|error`. CI (quality-gates guardrails job) runs `--scope changed --fail-on error` so only NEW anti-patterns on changed lines block the PR. Local: `npm run design:audit:changed`. Falls back to full scan (fail-open) when git/base is unresolvable.
- **Edit-time design-audit hook (2026-09-01):** `.opencode/plugin/design-audit-hook.js` runs the scanner on a frontend file immediately after an agent `edit`/`write` and appends error-severity findings to the tool output, so violations surface while the agent is still working. Fail-open, silent on clean/warn-only, kill switch `DESIGN_AUDIT_HOOK=0`. Requires opencode restart after plugin changes.
- **Render-integrity checklist (2026-09-01):** `.github/skills/ui-layout-audit/references/render-integrity-checklist.md` — R1–R8 taxonomy for render-layer layout defects code review can't catch (inline/block stacking, baseline traps, dead declarations). Origin: terms-page pill incident. Write-time rules L1–L5 live in `mini-program-frontend-excellence/references/taro-ui-framework.md` §WXSS layout semantics.
...
- `docs/design/registration-ceremony-spec-20260817.md` — 「订座」registration/payment ceremony spec (P1 narrative rework)
