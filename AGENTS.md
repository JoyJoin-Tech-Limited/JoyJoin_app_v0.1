...
- Use `deployment/docker-compose.staging.yml` to run an isolated staging API + Postgres on the production host (port `5001`).
- `deployment/nginx/joyjoin.conf` already includes the `staging.joyjoinapp.com` server block.
...
- Bundled assets: tab-icons, joyjoin-logo, joyjoin-logo-tab, tab-bar-notch-bg, custom-tab-bar, archetype spritesheet (subpackage), locally bundled icon tiers with retina (@1x/@2x/@3x via `JoyJoinIcon`): mood, chemistry, status, category, intent, expression (rating faces), semantic (info labels), **ui** (profile/settings list icons); **archetype heads ship as bare `.webp` filenames** (no `@3x` suffix — see `@3x` pitfall below) and are bundled locally with CDN fallback copies. Landing phase icons (6), empty states, QR code, auction coin icons, Xiaoyue loading + welcome expressions, Xiaoyue mascot sprite-sheet fallback, Quicksand + Alimama minimal fonts. CDN-only: archetype full-body images (WebP primary + PNG fallback), **phase emblems**, reaction, reveal, achievement, matching heroes, promo banners, Lovart illustrations, icebreaker backgrounds, celebration images, extra Xiaoyue expressions, mini-script heroes, ceremony heroes, milestone badges.
- **`CDN_ICON_TIERS` pitfall (2026-06-16 / updated 2026-06-17):** `packages/shared/src/iconSystem/emojiToIconMap.ts` lists tiers that resolve via CDN. Tiers **not** in this set resolve via `localAsset()` with root-relative `/assets/icons/<tier>/` paths against bundled `src/assets/icons/<tier>/`. Current CDN tiers: `phase`, `reaction`, `reveal`, `achievement`. `intent` was incorrectly in `CDN_ICON_TIERS`, causing broken CDN paths and emoji fallback on pool-registration and onboarding intent grids; it is now bundled locally. `category` was also removed from `CDN_ICON_TIERS`; category icons now resolve through bundled `src/assets/icons/category-icons/` and are pre-warmed via `usePreloadCategoryIcons` before the interest-heat picker renders. `phase` moved **into** `CDN_ICON_TIERS` on 2026-06-16 because the full phase-icon set is too large for the main package; only a curated landing-page subset is bundled locally under `/assets/landing-phase-icons/`. **`ui` tier added 2026-06-17** for profile/settings list icons (`/assets/icons/ui/`) and is bundled locally. `chemistry` and `ui` icons were previously uploaded to CDN but are no longer kept there (they load from the local bundle). Keep critical subpackage UI chrome out of `CDN_ICON_TIERS`. **Local bundled icons load eagerly by default** (`JoyJoinIcon` disables `lazyLoad` for non-CDN tiers) because WeChat's lazy-load is unreliable for local assets inside subpackages and can fall back to emoji.
...
- Failure rollback: `handleTabTap`/`handleCenterTap` roll back on `wx.switchTab` `fail`. Success/fail tracked via `wx.reportAnalytics`. 180ms double-tap debounce prevents rapid re-entry
- Sliding active pill: a single shared `.joy-custom-tab-bar__active-pill` translates via `transform: translateX(...)` (220ms GPU-only transition); per-item active backgrounds/scale animations were removed to reduce per-switch frame cost
