...
**Profession input overlay (2026-05-24, hardened 2026-06-07):**
...
- **Performance (2026-06-07):** Message list is memoized with `useMemo` to prevent re-creation on `inputValue` keystrokes. `will-change: transform` on `&__message` and `will-change: transform, opacity` on `&__reveal-tag` for GPU compositing; reset to `will-change: auto` on low-end. Keyboard scroll-to-bottom uses declarative `bottomAnchorKey` state + `ScrollView.scrollIntoView` on a 1px anchor.
- **Operational analytics (2026-06-07):** Seven onboarding analytics events track profession chat health: `profession_chat_classification_success` (tag count, confidence, source), `profession_chat_classification_fallback` (error type, input length, `timedOut` flag), `profession_chat_echo_suppressed`, `profession_chat_offline_blocked`, `profession_chat_skipped`, `profession_chat_retry_tapped`, `profession_chat_max_send_reached`.
...
- **Keyboard-safe:** Uses `onKeyboardHeightChange` + manual padding; `adjustPosition={false}` and `cursorSpacing={32}` on input to prevent native scroll fighting.
