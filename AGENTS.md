...
> Compact instructions for AI coding agents. Last updated: 2026-07-09
...
**Geo Services (2026-06-26):** Two server endpoints in `apps/server/src/routes/domains/geo.ts` using Tencent Maps WebService API (`TENCENT_MAP_KEY`):
- `POST /api/geo/reverse-geocode` — converts GPS coordinates to city/district (bounds-based fallback when API unreachable)
...
**In-session tier management (2026-07-07):** During `warmup`, the host sees a status chip (`当前 · 深度畅聊 · 60min`) in the `WarmupPhaseView` and can open a host `⋯` menu → "更换模式" to bring up `IcebreakerTierSheet`. Tier changes are host-only and locked to `waiting`/`warmup`; switching from a preset tier to `custom` (or vice versa) requires a double-confirm `Taro.showModal`. Analytics: `icebreaker_session_tier_changed`.
...
**Squad unboxing card-deck reveal (2026-06-28, redesign v2 2026-06-29, full-marks hardening 2026-07-08, gift-box tap 2026-07-09):** `pages/squad-unboxing/index` renders revealed members as a fanned `SquadDeckStage` of collectible `TeammateCard` components on a light warm gradient background (`$color-bg-tint-cream` → `$color-bg-tint-pink` → `$color-bg-tint-purple`). Tap-to-focus drives an inline `TeammateCardDetail` (no separate bottom panel); the inline detail is centered below the fixed stage via `Taro.createSelectorQuery` math so it never overlaps the focused card. Full-body archetype images load from CDN via `ARCHETYPE_ASSET_MAP`; missing archetypes use `MissingArchetypePlaceholder` (JoyJoin logo mark, no initials fallback). Each card border/shadow is tinted by the member's archetype color via `@shared/archetypeColors`. A stacked card-back pattern (`/assets/lovart/squad/squad-card-back-pattern-20260628-v1.webp`) decorates the deck and is preloaded during the `shaking` state. After reveal, Xiaoyue appears in an AI 团魂 bubble with a `TypewriterText` reveal capped at 3 s, showing the archetype-mix line and group-theme companion. The action dock shows `确认出席` (primary), `生成队伍海报` (share-poster CTA copy: "保存这桌记忆") + `查看活动详情` (secondary row), and `稍后再看`. Confirming attendance triggers a Xiaoyue success overlay before routing to the event/pool group. The `shaking`/`revealed` flow persists per group via `jj_revealed_${groupId}` storage. Scroll container uses `calc(100vh)` before `100dvh` for broad device support. **Gift-box interaction (2026-07-09, tap copy 2026-07-11):** the gift box itself is now tappable in `ready` state (`role="button"`, `aria-label="轻点打开礼盒，查看今晚的同桌"`, pressed-state feedback); the `DragRevealRibbon` is anchored below the gift box (not inside the copy card) and uses conditional `catchMove` so drag mode blocks scroll while tap-fallback mode allows vertical scroll. **Composed ready hero (2026-07-11, audit-hardened 2026-07-12, feature-flagged `socialSquadComposedHeroEnabled`, default `false`):** when enabled, the ready state replaces the mascot header + ribbon + copy card with a single composed unit — `XiaoyueHostImage` (CDN `/assets/lovart/squad/squad-host-xiaoyue.webp` → bundled local `squad-host-xiaoyue-fallback.webp` → gradient skeleton; the fallback is copied to `dist/` via `config/index.ts` and kept by `clean-cdn-assets.mjs`, CDN primary stays CDN-only) presenting the existing `BlindBoxVisual`, one headline/supporting beat (`type-headline` 56rpx — the 11-glyph title must stay single-line), and a glued `轻点打开` pill that reuses `handleOpenBox('box')`; the ribbon and old copy card are hidden in this mode, and revealed chemistry/pair labels render as temperature words (`getChemistryWord`/`getPairChemistryWord`) instead of naked integers. The whole stage (host + box) is one tap target via `squad-unboxing__stage-tap-layer` (`role="button"`, `aria-label="轻点打开礼盒，查看今晚的同桌"`); the stage body drops its own handlers in composed mode and receives press feedback through the `+ .squad-unboxing__stage-body` sibling selector. Entrance is a staggered child reveal (eyebrow/title/supporting/gesture at 0/60/120/180ms, transform+opacity); under `prefers-reduced-motion` the host duck-away becomes a 150ms opacity crossfade and the gesture press scale applies instantly. The hero is eager-loaded (WeChat lazy-load unreliable above the fold) and preloaded via `SQUAD_UNBOXING_PRELOADS`. Analytics: `squad_unboxing_box_tap`, `squad_unboxing_card_focus`, `squad_unboxing_confirm_attendance_tap`, `squad_unboxing_confirm_attendance_success`, `squad_unboxing_confirm_attendance_error`, `squad_unboxing_share_poster_tap`, `squad_unboxing_bubble_reveal_complete`, `squad_unboxing_box_open_milestone`. Server: `POST /api/pool-groups/:groupId/confirm-attendance` is idempotent (clean 200 when already `confirmed`).
