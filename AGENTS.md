...
**Gathering room (集结房间, 2026-08-07):** pre-event online anteroom for matched pool groups — subpackage `pages/gathering-room`, REST `GET /api/pool-groups/:groupId/room-state`, WS presence via `ROOM_*` events (5s leave grace, 2s poke throttle), attendance source of truth `event_attendance`. Ships dark behind `gatheringRoomEnabled` (env `GATHERING_ROOM_ENABLED`, default `false`); room art is `room-composite-v2.webp` (repaired 2026-08-27 for window panel mis-registration; v1 retained for rollback; `ROOM_ART_READY = true`) with CSS placeholder as fallback. Avatars reuse existing V2 `PixelAvatarComposite` with each member's equipped outfit/items. **Freeze fix (2026-08-12):** the page previously had 4 early returns BEFORE 3 hooks — first room render crashed React mid-mount and froze the loading screen forever; all hooks now run unconditionally above every early return. Never place a hook below an early return on this page (`rules-of-hooks` is not yet a CI gate). Weekday copy derives from `formatMeetDayLabel(eventDateTime)` — the old hardcoded 周六见 lied about non-Saturday events. **Polish rounds (2026-08-15):** two passes added entrance/presence crossfades, lamp-breathe layer, scene→CTA gradient scrim, first-visit hint, compact-viewport screenshot coverage, and removed placeholder CSS room layers; the avatar squash (`$pixel-avatar-squash: 0.85` on `PixelAvatarComposite`) and gender-neutral starter-outfit v2 direction also apply here. **Device fix (2026-08-17):** room art was invisible on device — `packOptions.include` is an upload allow-list and the bundled art matched no pattern, so the uploaded package dropped it; art now loads via `localAsset()` with an include regexp added. Absent members no longer pile dimmed avatars at the door — they show only the held-place name card at their seat; live arrivals walk in from the door. Details: `docs/agent-context/gathering-room.md`.
