...
**City Unlock v0.1 (2026-05-19, updated 2026-06-10):** Server-owned city expansion tracking via `user_city_interests` + `city_unlock_progress`. Threshold: 50 interested users triggers `collecting` → `researching` status transition + WeCom ops notification. Atomic count updates via Drizzle `sql` expressions (race-safe). Frontend: GPS auto-filter handles Shenzhen silently; `CityUnlockFeedCard` (bottom of pool list, with Xiaoyue mascot) is the sole entry point for city interest; `CityPickerSheet` (Xiaoyue mascot header, search + hot-city grid + full city list, scroll-to-selected, analytics-instrumented); progress page at `pages/city-unlock/index`. Admin report: `GET /api/admin/cities/unlock-report`. Gentle banner (`CityUnlockBanner`) removed 2026-06-10 — replaced by GPS auto-filter + feed card.
...
- **Decode readiness**: `useSpriteReadiness` hook gates slot animation start until the spritesheet is confirmed decoded (500ms timeout). `backgroundColor` fallback (archetype accent soft) prevents blank circles. Skip logic consolidated into a single early gate in `results/index.tsx`: `shouldReduceMotion || prefersReducedMotion || deviceTier.isDegradation || !personalitySlotAnimationEnabled`. Degradation-tier devices skip the spinning reel and render the result card immediately; mid-spin frame-budget mitigation (fewer particles, shorter glow, emergency jump-to-result) remains for primary-tier devices when the spin is already underway.
- **Mascot sprite fallback (2026-06-13):** `XiaoyueSpriteAnimator` loads mascot sprite sheets from CDN via `cdnAsset()` and falls back to the bundled local WebP in `/assets/mascot/<state>.webp` on `onError`. This covers offline or stale-CDN cases without shipping PNG sprite sheets.
...
- Bundled assets: tab-icons, joyjoin-logo, joyjoin-logo-tab, tab-bar-notch-bg, custom-tab-bar, archetype spritesheet (subpackage), locally bundled icon tiers with retina (@1x/@2x/@3x via `JoyJoinIcon`): mood, chemistry, status, category, intent, expression (rating faces), semantic (info labels); **archetype heads ship as bare `.webp` filenames** (no `@3x` suffix — see `@3x` pitfall below). Landing phase icons (6), empty states, QR code, auction coin icons, Xiaoyue loading + welcome expressions, Xiaoyue mascot sprite-sheet fallback, Quicksand + Alimama minimal fonts. CDN-only: archetype full-body images, **phase emblems**, reaction, reveal, achievement, matching heroes, promo banners, Lovart illustrations, icebreaker backgrounds, celebration images, extra Xiaoyue expressions, mini-script heroes, ceremony heroes, milestone badges.
- **`CDN_ICON_TIERS` pitfall (2026-06-16):** `packages/shared/src/iconSystem/emojiToIconMap.ts` lists tiers that resolve via CDN. Tiers **not** in this set resolve via `localAsset()` with root-relative `/assets/icons/<tier>/` paths against bundled `src/assets/icons/<tier>/`. Current CDN tiers: `phase`, `reaction`, `reveal`, `achievement`. `intent` was incorrectly in `CDN_ICON_TIERS`, causing broken CDN paths and emoji fallback on pool-registration and onboarding intent grids; it is now bundled locally. `category` was also removed from `CDN_ICON_TIERS`; category icons now resolve through bundled `src/assets/icons/category-icons/` and are pre-warmed via `usePreloadCategoryIcons` before the interest-heat picker renders. `phase` moved **into** `CDN_ICON_TIERS` on 2026-06-16 because the full phase-icon set is too large for the main package; only a curated landing-page subset is bundled locally under `/assets/landing-phase-icons/`. Keep critical subpackage UI chrome out of `CDN_ICON_TIERS`.
...
**Custom tab bar geometry (2026-06-04 / visual polish 2026-06-05 / hardened 2026-06-11):**
...
- State sync: 50ms debounce + shallow diff in native `syncState`; badge updates use WeChat path syntax (`leftTabs[idx].badgeCount`) to avoid array reconstruction and icon flicker
...
- Failure rollback: `handleTabTap`/`handleCenterTap` roll back on `wx.switchTab` `fail`. Success/fail tracked via `wx.reportAnalytics`. 300ms double-tap debounce prevents rapid re-entry
- Lifecycle hygiene: `detached` clears `_syncTimer`, `_showTimer`, and `_tapDebounceTimer` to prevent leaks. `pageLifetimes.show` safety net handles swipe-back rollback when optimistic state survives page hide
...
- Low-end gating: `wx.getSystemInfoSync().benchmarkLevel <= 15` disables all animations via `.joy-custom-tab-bar--low-end` class (WXSS overrides). `will-change` hints are reset to `auto` on low-end + reduced-motion to free GPU layers.
...
- **`JoyJoinIcon`** (`apps/mini-program/src/components/ui/JoyJoinIcon.tsx`) — proprietary icon renderer replacing raw emoji on primary UI surfaces. Props: `emoji`, `size?`, `tier?`, `className?`, `style?`. 4-tier fallback chain (no mapping → require fail → load fail → native emoji). Composite tier-aware lookup via `packages/shared/src/iconSystem/emojiToIconMap.ts` (same Unicode emoji can resolve to different assets per context; `CDN_ICON_TIERS` controls CDN vs bundled resolution). Includes fade-in + brand motion-curve load animation, reduced-motion support, shimmer placeholder, and `alt` accessibility. Use for reactions, categories, intents, achievements, chemistry badges, status icons, phase emblems, rating faces, and info labels.
...
**Mini-program profile components (2026-05-24 / layout hardened 2026-06-04 / redesign shipped 2026-06-16):**
...
**ScrollView inside flex parent (2026-06-04 / drawer hardened 2026-06-05, updated 2026-06-10):** A `<ScrollView>` in a flex container will not scroll unless the parent has `min-height: 0` or explicit height. The `LocationFilterDrawer` fixed a scroll-lock bug by adding `min-height: 0` to the flex parent, adding WeChat's `enableFlex` prop to the `<ScrollView>`, removing a conflicting inline `height: 100%`, replacing the unreliable `70dvh` max-height with `calc(980rpx - $tab-bar-root-height)`, and removing conflicting `overflow-y` / `webkit-overflow-scrolling` CSS. The surface uses `bottom: $tab-bar-root-height` to avoid being hidden behind the custom tab bar. `catchMove` must be placed only on the backdrop — putting it on the outer container intercepts all touch events and blocks scroll.
