...
- Recommended: Yes. Tested on ≥2 devices: one Android (Snapdragon or MediaTek) + one iPhone (iPhone 15 or 16). Each platform has distinct WebView behavior. iOS WeChat does not expose `benchmarkLevel`; use the model/system heuristic in `apps/mini-program/src/hooks/useDeviceTier.ts` for tier detection.
