...
- **NPC/schedule:** NPC 目录支持持续新增数字动物角色，不设 5 位数量上限，也不使用封闭物种名单。内置种子仍提供阿浪、栗子、默默、拾柒、阿团作为首批角色；每位 NPC 使用运营配置的固定星期、随机地点和随机 1–2 班。单班 180–300 分钟（3–5 小时），09:00–21:00，同 NPC 间隔至少 90 分钟，生成器均衡上午/下午/晚上。没有可用安全班次时允许当天不出现，不展示未来排班，也不预约或加班。
...
- **My Image:** 12 个 canonical V4 动物使用 V2 纸娃娃分层渲染：每人格仅 1 张 512×768 透明基础身体图，基础身体固定穿着不可脱的贴身背心与安全短裤；每件初始装备仅 1 张裁边透明 WebP，并由 manifest 的 512×768 画布锚点放到身体上，同一张图同时用于衣橱缩略图和穿戴层。当前总量固定为 12 张身体 + 48 张初始装备层，不为穿搭组合生成整人合成图。四槽为上装/下装/鞋履/配饰，可独立脱下；预览只改草稿，必须显式“保存形象”，穿脱/保存/库存均保留真实服务端状态。五档旋转以客户端分层透视与景深视差实现，只称伪 3D，不宣称真实 360° 模型。以后新增衣服只增加一张透明装备图与锚点/景深元数据。
...
- **Venue disclosure gating:** Event location is hidden while registration status is `pending`/`registered`/`upcoming` ("报名成功" phase) and also while a matched group is awaiting venue assignment (raw `matched` without `venueName`). The card always shows event type + city/district. Venue is disclosed only after the server-derived `displayStatus` becomes `confirmed` or `venue_unlocked`. Terminal pool states (`completed`/`cancelled`) override the registration-level status.
...
**46. Squad Unboxing Tap-to-Reveal** 🂡 *(2026-07-14; committed `a6a160dc8` 2026-07-15)*
- **Face-down deal + tap-to-reveal:** Cards in the revealed squad fan now deal **face-down** and flip face-up via controller-owned `pages/squad-unboxing/squadFlipState.ts` — the user's own card auto-flips on deal (`auto_me`), other cards flip on tap (`tap`), and a live-count reveal-all chip flips every remaining card (`reveal_all`). Per-card local flip state remains banned.
- **One-step focus + dock narration:** Flipping/focusing a card narrates the member intro in the Xiaoyue dock bubble (burst > member > tease > soul chain, `buildFocusedMemberBubbleText` + `TypewriterText`). Face-up taps resolve via `resolveCardFocusInteraction`: focus a different card, fast-forward the running typewriter, or dismiss a focused card (unfocus, emits `squad_unboxing_card_detail_dismiss`). The inline `TeammateCardDetail` panel is retired; the tease bubble ("桌友卡都扣好了，轻点翻开…") avoids spoiling unflipped members.
- **Upstream merge semantics:** Sibling dimming on focus was removed (upstream `13b51fcb6`) — unfocused cards keep their full-opacity fan pose; `squad_unboxing_card_detail_dismiss` was reinstated as the resolver dismiss action (`a6ea57284`); member introductions use the 5-arg builder (`7d209b6de`). The center-card auto-peek and session-level front holo are retired (tests lock their absence); a one-time shimmer sweep plays across the face-down backs.
- **Hardening:** Background-during-deal recovery notifies the flip session on settle; `auto_me` is guarded to roster-visible ids; connection-point pills strip wrapping punctuation; deal landing haptics are throttled to ≥80ms stagger; the reveal-all chip meets the 88rpx tap-target floor; the typewriter is gated by reduced-motion/degradation-tier.
- **Every card has a hook + focused caption (2026-07-16):** The card-face info grid is a strict 4-row layout (name / archetype / `age·gender` / 1 hook pill). The hook pill shows the viewer's strongest connection point when one exists, else falls back to the member's top interest in a neutral-outline style (`buildInterestHookText`) — no card deals a bare 3-row face. Industry and education never appear on fan-covered cards (the overlap leaves only ~126rpx of visible text); they surface in the focused-card narration bubble (education clause: `{education}学历` right after industry), the face-up aria-label, and the focused card's art-zone bottom caption (`本科 · 互联网产品` overlay). Privacy flags (`ageVisible`/`industryVisible`/`educationVisible`) silently omit fields. `relationshipStatus` and `intent` are deliberately never shown on card faces (PM decision: blind-box table, not a dating catalog).
...
- **Sprint D — Auction Enhancement (countdown retired 2026-07-29):** Host-closed lots with persistent bid history (`auctionBidHistory`), outbid toast notifications, archetype-aware lot generation via `buildArchetypeContext` → prompt injection, lot emoji categorization fallback, CardFlip lot reveal, ParticleBurst win celebration, and IdentityReveal high-bidder spotlight. Negative balance guards are preserved; `auctionLotStartedAt` remains compatibility-only and does not drive gameplay.
...
- **Shared selection logic:** `toggleIntentValue(selected, value, { maxExplicit })` helper in `packages/shared/src/constants.ts` centralizes `随缘`/flexible coexistence rules. **Uncapped by default** (2026-08-05): users may pick any number of explicit intents; selecting every explicit option auto-collapses the selection to `随缘`. Callers that still enforce a limit pass `{ maxExplicit: N }` explicitly; it returns `null` when an explicit cap would be exceeded. **All mini-program surfaces uncapped (2026-09-07):** pool-registration (since 2026-08-05), onboarding `essential-data`, and edit-profile all use the uncapped default — selecting the 6th explicit intent collapses to 随缘 with `haptics('medium')`; `essential-data` adds a mascot reaction line and footer copy 「可以多选，全选了就交给悦仔安排。」. Covered by unit tests in `packages/shared/src/__tests__/intents.test.ts`. Used by pool-registration, onboarding `essential-data`, and edit-profile.
...
- **Completion feedback:** Pool-registration previously showed a muted completion pill "已经选满 3 个期待，先聚焦这些方向" when the explicit-intent cap was reached — removed with the cap (2026-08-05); selection feedback is now the mascot intent-feedback copy in `PoolRegistrationMascotSection`.
...
**37. Squad Unboxing Drag-to-Reveal Ribbon** 🎁 *(2026-06-08; gift-box tap + ribbon anchor 2026-07-09)*
...
- **Gift-box tap entry point (2026-07-09):** the gift box itself is now tappable in `ready` state (`role="button"`, `aria-label="点击拆开礼盒"`, pressed-state feedback). It triggers the same `shaking` → `revealed` flow as the ribbon, and is logged via the new `squad_unboxing_box_tap` analytics event. The `BlindBoxLid` overlay stays `pointer-events: none` so taps pass through to the box body.
- **Ribbon anchor (2026-07-24):** `DragRevealRibbon` is rendered below the header (not below the box) since the standalone copy card was deleted. The ribbon uses conditional `catchMove`: drag mode blocks parent `ScrollView` scrolling; tap-fallback / low-end mode allows vertical scroll. The white instruction card was removed along with the shaking copy card — the ready state is now header-only (eyebrow `第N组 · M位同桌`, title `你的桌友来了`, tagline `悦仔把今晚的同桌装进盒子了，就等你亲手拆开。`).
...
- **Performance:** RAF-throttled progress updates (`requestAnimationFrame` batching) prevent re-render thrashing. `transform` + `opacity` only — no layout-triggering properties. Conditional `catchMove` prevents parent `ScrollView` interference during horizontal drag while preserving scroll in tap-fallback mode. `+0.23kB` gzipped.
- **Accessibility:** `role="slider"`, `aria-valuenow` (0–100), and dynamic `aria-label` ("向右滑动，拆开惊喜" / "点击拆盒") on the ribbon. Touch target is full track width at 88rpx height. Gift box is a separate `role="button"` with `aria-label="点击拆开礼盒"`.
...
- **Analytics:** `POST /api/analytics/squad-unboxing` endpoint (120 req/min rate limit) tracks `squad_unboxing_box_tap`, `squad_unboxing_reveal_drag` (with `progress` %), and `squad_unboxing_reveal_tap` (with `fallbackReason`). Fire-and-forget, silent fail.
...
**Per-pool gender balance (operator-configured, wired 2026-07-14):** Each event pool carries `genderBalanceMode` — `none`, `soft` (default; exact male/female balance earns a bounded group-diversity bonus, formation never blocked), or `hard` (each committed group must meet the pool's `minFemaleCount`/`minMaleCount` floors, enforced at the commit gate and during redistribution). `genderRestriction="女性"` (all-female events) remains the strongest control and implies `none`. Operators configure these per pool in the admin portal (`性别平衡` section); changes are audit-logged.
