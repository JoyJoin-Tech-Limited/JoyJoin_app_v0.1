...
- **Ribbon anchor:** `DragRevealRibbon` is rendered below the gift box (not inside the white instruction card) so the visual affordance is always adjacent to the object it opens. The ribbon uses conditional `catchMove`: drag mode blocks parent `ScrollView` scrolling; tap-fallback / low-end mode allows vertical scroll.
