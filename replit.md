...
- **Gamified Personality Assessment (V4 Adaptive):** An adaptive 97-question bank (including 6 forced-choice tradeoff questions + 5 targeted differentiation questions Q90-Q94) dynamically selecting 8-16 questions based on real-time confidence, integrating with onboarding. Features an adaptive engine with 3-level question difficulty, real-time archetype predictions, and milestone encouragement. Current accuracy: 58% exact match, 75% similar match (target: 70%/85%, improved from 54%/65% baseline via systematic bias reduction). Uses explicit targetPairs metadata on differentiation questions for reliable prioritization when confusable archetypes are detected. All 97 questions have Xiaoyue feedback entries.
