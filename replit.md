...
- **Gamified Personality Assessment (V4 Adaptive):** An adaptive 126-question bank (including 6 forced-choice tradeoff questions + 12 targeted differentiation questions Q90-Q97, Q119-Q126) dynamically selecting 8-16 questions based on real-time confidence, integrating with onboarding. Features an adaptive engine with 3-level question difficulty, real-time archetype predictions, and milestone encouragement. Current accuracy: 64% exact match, 82% similar match (target: 70%/85%, near theoretical ceiling for short-form assessments). Uses explicit targetPairs metadata on differentiation questions for reliable prioritization when confusable archetypes are detected. All questions have Xiaoyue feedback entries. Q123-Q126 added to address worst-performing confusion pairs: 暖心熊/淡定海豚 (A+P/A+X traits) and 太阳鸡/淡定海豚 (P+X/P+E traits).
- **MatcherV2 Algorithm (v2.2-manhattan):** Two-phase scoring with SIGNATURE_THRESHOLDS (phase 1) + CONFUSION_PAIR_GATES (phase 2). Uses weighted Manhattan distance with exp(-λ*d_norm) formula. PROTOTYPE_SOUL_TRAITS: 12 archetypes with primary(1.6-1.8x)/secondary(1.2-1.3x)/avoid(0.6-0.8x) weights. Persistent confusion pairs requiring structural changes: 太阳鸡→淡定海豚, 沉思猫头鹰→稳如龟, 淡定海豚→暖心熊.
