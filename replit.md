...
- **风格谱系 (Style Spectrum) System:** New presentation model showing primary archetype + adjacent styles rather than single precise match. `getStyleSpectrum()` API returns primary/adjacentStyles/spectrumPosition. Mobile-first `StyleSpectrum` component with hero card, swipeable orbit chips, and animated spectrum sliders.
