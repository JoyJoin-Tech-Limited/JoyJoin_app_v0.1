...
- **AI Chat Registration:** New AI-powered conversational registration flow using a character-based AI (小悦) for a more engaging user onboarding experience, extracting user information securely server-side. Features include:
  ...
  - **Expert Evaluation System:** Comprehensive AI expert assessment framework for quality evaluation. Features include:
    - **10 AI Expert Personas:** NLP专家(李明远), 对话系统专家(王雅琪), UX专家(陈思雨), ML专家(张浩然), 产品专家(林晓婷), 心理学专家(赵心怡), 语言学专家(周语辰), AI伦理专家(孙雨轩), 创业专家(刘创业), 架构师(吴代码)
    - **7 Weighted Evaluation Dimensions:** Inference Accuracy (20%), Redundancy Reduction (20%), Conversation Coherence (15%), User Experience (15%), Edge Case Handling (10%), Response Quality (10%), Technical Robustness (10%)
    - **Simulated Expert Evaluation:** DeepSeek-powered expert role-playing with structured JSON output for professional assessments
    - **Human Expert Questionnaire:** Comprehensive evaluation form template with rating scales, reasoning fields, and improvement suggestions
    - **Grading System:** A+/A/B+/B/C/D/F grades based on aggregated weighted scores
    - **API Endpoints:** `GET /api/inference/expert-personas`, `GET /api/inference/questionnaire`, `GET /api/inference/scenario-stats`, `POST /api/inference/expert-evaluation` (admin), `POST /api/inference/full-evaluation` (admin)
    - **Key Files:** `server/inference/expertEvaluation.ts`, `server/inference/scenarios.ts`
