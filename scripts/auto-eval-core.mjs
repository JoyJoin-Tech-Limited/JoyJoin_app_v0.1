...
const FRONTEND_CATEGORIES = new Set(['user-client', 'admin-client', 'mini-program']);
...
function classifyPath(filePath) {
  ...
  if (filePath.startsWith('apps/user-client/')) return 'user-client';
...
function runSecurityModule(repoRoot, changedFiles, timeoutProfile) {
  ...
  const result = runCommand('node', ['scripts/check-guardrails.mjs'], {
    cwd: repoRoot,
