...
for (const p of personas) {
  ...
  console.log(`${p.name}: ${p.expectedArchetype} → ${result.assignedArchetype} ${status}`);
}

console.log(`\nExact: ${exact}/${personas.length} = ${(exact/personas.length*100).toFixed(1)}%`);
console.log(`Similar+Exact: ${exact+similar}/${personas.length} = ${((exact+similar)/personas.length*100).toFixed(1)}%`);
