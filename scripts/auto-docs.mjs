 ...
 *
 * Usage:
 *   node scripts/auto-docs.mjs                         # analyze recent changes (default: 20 commits)
 *   node scripts/auto-docs.mjs --commits 50            # look back further
 *   node scripts/auto-docs.mjs --range HEAD~10..HEAD   # explicit range
 *   node scripts/auto-docs.mjs --pr                    # create PR (CI mode)
 *   node scripts/auto-docs.mjs --wecom                 # send WeCom notification
 *   node scripts/auto-docs.mjs --scan-all              # scan full codebase for gaps
 *
 * Exit codes:
...
const SUBSYSTEM_MAP = [
  ...
  { pattern: 'apps/user-client/src/', name: 'Web User Client' },
...
/**
 * Generate a README for a server domain directory
 */
function generateDomainDoc(mapping) {
  const sourcePath = path.join(process.cwd(), mapping.source);
  try {
    const entries = fs.readdirSync(sourcePath);
    const files = entries.filter(e => e.endsWith('.ts') && !e.endsWith('.test.ts') && !e.endsWith('.spec.ts'));
...
async function createDocPR(analysis) {
  ...
  for (const gap of analysis.gaps) {
    if (!gap.docExists) {
      const content = generateDomainDoc(gap.mapping);
