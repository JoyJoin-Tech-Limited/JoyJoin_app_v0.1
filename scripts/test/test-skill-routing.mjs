 ...
 *
 * Usage:
 *   node scripts/test/test-skill-routing.mjs
 *
 * Exit codes:
...
test('where does component go → frontend-component-architecture', () => {
  const r = routeSkill({ ask: 'Where does this ProfileCard component go — should it be in packages/shared or in mini-program?' });
