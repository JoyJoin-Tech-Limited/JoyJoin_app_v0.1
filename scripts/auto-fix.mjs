 ...
 *
 * Usage:
 *   node scripts/auto-fix.mjs                        # dry run (show what would fix)
 *   node scripts/auto-fix.mjs --pr                   # create fix PR
 *   node scripts/auto-fix.mjs --pr --wecom            # fix PR + WeCom notify
 *   node scripts/auto-fix.mjs --commits 20            # look back further
 *
 * Environment:
...
const BUG_PATTERNS = [
  {
    ...
    getFix(lineText, lineNum) {
      // For empty catch(e) {} on same line: replace with catch(e) { logger.error(...); }
