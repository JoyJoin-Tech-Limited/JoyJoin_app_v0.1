 ...
 *
 * Usage:
 *   node scripts/auto/auto-triage.mjs                          # scan all open PRs + issues
 *   node scripts/auto/auto-triage.mjs --dry-run                # preview without applying
 *   node scripts/auto/auto-triage.mjs --prs                    # only PRs
 *   node scripts/auto/auto-triage.mjs --issues                 # only issues
 *   node scripts/auto/auto-triage.mjs --wecom                  # notify WeCom
 *   node scripts/auto/auto-triage.mjs --number 42 --type pr    # triage one item
 *
 * Exit codes:
...
const AREA_RULES = [
  ...
  { patterns: [/^apps\/web\//], label: 'area:web', description: 'Web user client changes' },
