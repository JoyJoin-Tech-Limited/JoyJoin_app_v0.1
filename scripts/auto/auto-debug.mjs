 ...
 *
 * Usage:
 *   node scripts/auto/auto-debug.mjs                         # inspect recent commits (default: 10)
 *   node scripts/auto/auto-debug.mjs --commits 20            # look back further
 *   node scripts/auto/auto-debug.mjs --range HEAD~5..HEAD    # explicit range
 *   node scripts/auto/auto-debug.mjs --pr                     # create PR if bugs found (CI mode)
 *   node scripts/auto/auto-debug.mjs --wecom                  # send WeCom notification
 *
 * Exit codes:
...
async function runQualityGates() {
  ...
  // Run guardrails
  console.log('\n🔒 Running guardrails...');
  const guardrails = spawnSync('node', ['scripts/check/check-guardrails.mjs'], {
  ...
  const depCheck = spawnSync('node', ['scripts/check/check-workspace-dependency-ownership.mjs'], {
    encoding: 'utf8', timeout: 30000,
  });
  results.push({
    name: 'dep-check',
