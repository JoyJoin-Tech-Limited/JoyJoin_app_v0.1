 ...
 *
 * Usage:
 *   node scripts/harness/harness-cost-tracker.mjs \
 *     --task-id=<id> \
 *     --tier=<1|2|3> \
 *     --agent=<name> \
 *     --input-tokens=<n> \
 ...
 *   node scripts/harness/harness-cost-tracker.mjs --batch=<json-path>
 *
...
function main() {
  ...
  if (!taskId || !tier || inputTokens === null || outputTokens === null || !modelTier) {
    console.error("Usage:");
    console.error("  node scripts/harness/harness-cost-tracker.mjs --task-id=<id> --tier=<n> --input-tokens=<n> --output-tokens=<n> --model-tier=<mini|standard|max> [--agent=<name>]");
    console.error("  node scripts/harness/harness-cost-tracker.mjs --batch=<json-path>");
    console.error("  node scripts/harness/harness-cost-tracker.mjs --report");
    process.exit(1);
  }
