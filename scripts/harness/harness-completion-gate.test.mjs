...
describe('Harness Completion Gate', () => {
  ...
  describe('Cross-app import detection', () => {
    it('fails on mini-program importing from admin-client', () => {
      ...
      fs.mkdirSync(path.join(tmpDir, 'apps', 'mini-program', 'src'), { recursive: true });
      fs.writeFileSync(
        path.join(tmpDir, 'apps', 'mini-program', 'src', 'page.tsx'),
        "import { AdminOnly } from '../../../admin-client/src/components/AdminOnly';\n"
      );
      spawnSync('git', ['add', 'apps/mini-program/src/page.tsx'], { cwd: tmpDir });
